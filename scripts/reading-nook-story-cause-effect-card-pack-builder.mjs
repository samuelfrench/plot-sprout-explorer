...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'reading-nook-story-cause-effect-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'reading-nook-story-cause-effect-card-pack')

function readingNookCauseEffectBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Reading-Nook-Story-Cause-Effect-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'reading-nook-story-cause-effect-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'reading-nook-story-cause-effect-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Reading Nook Cause-and-Effect Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Reading Nook Cause-and-Effect Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Cause prompt', card.causePrompt),
    renderField('Effect prompt', card.effectPrompt),
    renderField('Because prompt', card.becausePrompt),
    renderField('So prompt', card.soPrompt),
    renderField('Chain prompt', card.chainPrompt),
  ...
  return `
    <section class="pack-page story-sequence-card-page">
      <div class="page-kicker">Cause-and-Effect Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.causeEffectSkill)}</div>
...
export function renderReadingNookStoryCauseEffectCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Reading Nook Cause-and-Effect Card source world slug: ${slug}`)
...
<html lang="en">
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Cause/effect move coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper cause/effect setup', source.adultGuide.paperCauseEffectSetup)}
        ${renderGuideCard('Story cause/effect coaching', source.adultGuide.causeEffectCoaching)}
      ...
      <p class="footer-note">Use one paper cause/effect card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen cause-and-effect-card worlds</h2>
      <p>Pick a world image, then connect one invented reading nook cause to one story effect.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Cause/effect routines</p>
      <h2>Choose the paper cause/effect rhythm</h2>
      <div class="routine-grid">${source.causeEffectRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home cause/effect slips</p>
      <h2>Finish one story cause-and-effect later</h2>
      <div class="slip-grid">${source.takeHomeCauseEffectSlips.map(renderSlip).join('\n')}</div>
...
export function loadReadingNookStoryCauseEffectCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = readingNookCauseEffectBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = readingNookCauseEffectBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = readingNookCauseEffectBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Reading-Nook-Story-Cause-Effect-Card-Pack.pdf',
    '- source/reading-nook-story-cause-effect-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = readingNookCauseEffectBuildPaths()) {
  const entries = [
    {
      name: 'Reading-Nook-Story-Cause-Effect-Card-Pack.pdf',
    ...
    {
      name: 'source/reading-nook-story-cause-effect-card-pack.html',
...
export async function buildReadingNookStoryCauseEffectCardPack(options = {}) {
  const { source, product, worlds } = loadReadingNookStoryCauseEffectCardPackBuildInputs()
  const paths = readingNookCauseEffectBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildReadingNookStoryCauseEffectCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
