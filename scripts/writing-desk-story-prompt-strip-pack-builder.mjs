...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'writing-desk-story-prompt-strip-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'writing-desk-story-prompt-strip-pack')

function writingDeskStripBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Writing-Desk-Story-Prompt-Strip-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'writing-desk-story-prompt-strip-pack.zip'),
    htmlPath: resolve(sourceDir, 'writing-desk-story-prompt-strip-pack.html'),
...
function renderStripFormat(format) {
  return `
...
function renderStripSlip(slip) {
  return `
    <article class="strip-slip">
...
function renderStrip(strip, index, worlds, imageMap) {
  const world = worlds.get(strip.worldSlug)
  if (!world) throw new Error(`Unknown Writing Desk Story Prompt Strip world slug: ${strip.worldSlug}`)
  const imagePath = imageMap.get(strip.worldSlug)
  if (!imagePath) throw new Error(`Missing Writing Desk Story Prompt Strip copied image for ${strip.worldSlug}`)
  const fields = [
    renderField('Front prompt', strip.stripFrontPrompt),
    renderField('Back prompt', strip.stripBackPrompt),
    renderField('Story seed', strip.storySeedPrompt),
    renderField('First line path', strip.firstLinePath),
    renderField('Revision nudge', strip.revisionNudge),
    renderField('Quiet option', strip.quietOptionLine),
  ].join('\n')

  return `
    <section class="pack-page strip-page">
      <div class="page-kicker">Paper Strip ${index + 1} | Ages ${escapeHtml(strip.ageBand)} | ${escapeHtml(strip.stripSkill)}</div>
      <h2>${escapeHtml(strip.title)}</h2>
      <figure>
        <img src="${escapeHtml(imagePath)}" alt="${escapeHtml(world.title)} illustration">
        <figcaption>${escapeHtml(world.title)}</figcaption>
      </figure>
      <p class="adult-note"><strong>Adult setup:</strong> ${escapeHtml(strip.adultSetup)}</p>
      <p class="kid-direction">${escapeHtml(strip.kidDirection)}</p>
      <div class="field-grid">${fields}</div>
      <p class="take-home-line">${escapeHtml(strip.takeHomeLine)}</p>
    </section>`
}

export function renderWritingDeskStoryPromptStripPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Writing Desk Story Prompt Strip source world slug: ${slug}`)
      return renderWorldCard(world, imageMap.get(slug))
    })
    .join('\n')
  const strips = source.strips
    .map((strip, index) => renderStrip(strip, index, worlds, imageMap))
    .join('\n')
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .guide-card, .small-card, .world-card, .strip-slip, .field-block {
        padding: 0.04in;
      ...
      .small-card p, .small-card li, .strip-slip p { margin-bottom: 0.022in; }
      .strip-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .strip-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Desk-clue story coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Strip setup', source.adultGuide.deskSetup)}
        ${renderGuideCard('Desk-clue story coaching', source.adultGuide.stripStoryCoaching)}
      ...
      <p class="footer-note">Use one paper strip at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Eighteen strip worlds</h2>
      <p>Pick a world image, then turn one invented desk clue into a story start.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page format-page">
      <p class="page-kicker">Strip formats</p>
      <h2>Choose the paper strip rhythm</h2>
      <div class="format-grid">${source.stripRoutines.map(renderStripFormat).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home desk strips</p>
      <h2>Finish one story clue later</h2>
      <div class="slip-grid">${source.takeHomeDeskStrips.map(renderStripSlip).join('\n')}</div>
...
export function loadWritingDeskStoryPromptStripPackBuildInputs() {
  ...
  for (const strip of source.strips) {
    if (!worlds.has(strip.worldSlug)) throw new Error(`Missing strip world record for ${strip.worldSlug}`)
    if (!imageMap.has(strip.worldSlug)) throw new Error(`Missing local copied image for strip world ${strip.worldSlug}`)
  }
  return {
    source,
    product,
...
function prepareBuildDirectory(paths = writingDeskStripBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = writingDeskStripBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = writingDeskStripBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Writing-Desk-Story-Prompt-Strip-Pack.pdf',
    '- source/writing-desk-story-prompt-strip-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = writingDeskStripBuildPaths()) {
  const entries = [
    {
      name: 'Writing-Desk-Story-Prompt-Strip-Pack.pdf',
    ...
    {
      name: 'source/writing-desk-story-prompt-strip-pack.html',
...
export async function buildWritingDeskStoryPromptStripPack(options = {}) {
  const { source, product, worlds } = loadWritingDeskStoryPromptStripPackBuildInputs()
  const paths = writingDeskStripBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderWritingDeskStoryPromptStripPackHtml(source, worlds, imageMap)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildWritingDeskStoryPromptStripPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
