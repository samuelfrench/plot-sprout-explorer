...
import {
  shelfMarkerStoryThemeCardPackProductSlug,
...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'shelf-marker-story-theme-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', shelfMarkerStoryThemeCardPackProductSlug)
...
function shelfMarkerStoryThemeBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Shelf-Marker-Story-Theme-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'shelf-marker-story-theme-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'shelf-marker-story-theme-card-pack.html'),
...
function assembleSourceFromLanes() {
  ...
  if (cardLanes.length !== 3 || !tools) {
    throw new Error('Batch63 shelf marker source lanes are incomplete.')
  }

  return {
    batchId: '2026-06-04-batch63',
    generatedAt: '2026-06-04',
    productSlug: shelfMarkerStoryThemeCardPackProductSlug,
...
export function loadShelfMarkerStoryThemeCardPackBuildInputs() {
...
function renderSlip(slip, index) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home theme slip</p>
      <h3>${escapeHtml(slip.title || `Theme slip ${index + 1}`)}</h3>
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Shelf Marker Story Theme Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  const image = imagePath
    ? `<img src="${escapeHtml(imagePath)}" alt="${escapeHtml(world.title)} illustration">`
    : '<div class="image-placeholder" aria-hidden="true"></div>'
  const fields = [
    renderField('Story question', card.storyQuestionPrompt),
    renderField('Repeated clue', card.repeatedCluePrompt),
    renderField('Character choice', card.characterChoicePrompt),
    renderField('Ending echo', card.endingEchoPrompt),
    renderField('Theme line', card.themeLinePrompt),
    renderField('Shelf marker note', card.shelfMarkerNotePrompt),
  ].join('\n')

  return `
    <section class="pack-page theme-card-page">
      <div class="page-kicker">Theme Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.themeSkill)}</div>
...
export function renderShelfMarkerStoryThemeCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Shelf Marker Story Theme Card source world slug: ${slug}`)
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .theme-card-page { font-size: 7.3px; }
      ...
      .theme-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .theme-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    <section class="pack-page cover-page">
      <div>
        ...
        <p>Family-safe printable writing pages for adult-led, offline theme practice.</p>
      </div>
    ...
    <section class="pack-page guide-page">
      ...
      <p class="footer-note">Use one story theme card at a time. Keep every theme line broad, invented, paper-only, and guided by an adult.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen theme card worlds</h2>
      <p>Pick a world image, then name the story question, repeated clue, character choice, ending echo, theme line, and shelf marker note.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Theme routines</p>
      <h2>Choose the story theme routine</h2>
      <div class="routine-grid">${source.themeRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home theme slips</p>
      <h2>Try one theme later</h2>
      <div class="slip-grid">${source.takeHomeThemeSlips.map(renderSlip).join('\n')}</div>
...
function prepareBuildDirectory(paths = shelfMarkerStoryThemeBuildPaths()) {
...
function copyPackAssets(source, paths = shelfMarkerStoryThemeBuildPaths(), options = {}) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
    const sourceImage = sourceImageForSlug(slug, options)
    if (!sourceImage) throw new Error(`Missing Shelf Marker Story Theme Card Pack source image for ${slug}`)
...
function writeReadme(source, paths = shelfMarkerStoryThemeBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = shelfMarkerStoryThemeBuildPaths()) {
  const entries = [
    {
      name: 'Shelf-Marker-Story-Theme-Card-Pack.pdf',
    ...
    {
      name: 'source/shelf-marker-story-theme-card-pack.html',
...
function assertValidBuildInput(source, product, worlds, sourceFilesRoot = root) {
  const sourceErrors = validateShelfMarkerStoryThemeCardPackSource(
    source,
    product,
    worldAgeRecords(worlds),
  )
  const laneErrors = validateShelfMarkerStoryThemeCardPackSourceFiles(source, sourceFilesRoot)
  const errors = [...sourceErrors, ...laneErrors]
  if (errors.length > 0) {
    throw new Error(`Shelf Marker Story Theme Card Pack source is invalid:\n${errors.join('\n')}`)
  }
}

export async function buildShelfMarkerStoryThemeCardPack(options = {}) {
  const inputs = options.source
    ...
    : loadShelfMarkerStoryThemeCardPackBuildInputs()
  const { source, product, worlds } = inputs
  const paths = shelfMarkerStoryThemeBuildPaths(options.outputDir ?? options.buildDir ?? buildDir)
  ...
  const html = renderShelfMarkerStoryThemeCardPackHtml(source, worlds, imageMap)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildShelfMarkerStoryThemeCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
