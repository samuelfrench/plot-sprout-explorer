...
import {
  bookendStoryEvidenceCardPackProductSlug,
...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'bookend-story-evidence-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', bookendStoryEvidenceCardPackProductSlug)
...
function bookendStoryEvidenceBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Bookend-Story-Evidence-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'bookend-story-evidence-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'bookend-story-evidence-card-pack.html'),
...
function assembleSourceFromLanes() {
  ...
  if (cardLanes.length !== 3 || !tools) {
    throw new Error('Batch64 bookend evidence source lanes are incomplete.')
  }

  return {
    batchId: '2026-06-04-batch64',
    generatedAt: '2026-06-04',
    productSlug: bookendStoryEvidenceCardPackProductSlug,
    ...
    cover: {
      ...
      subhead:
        'Sixteen paper bookend cards help writers support fictional story claims with clue details, because lines, evidence sentences, and bookend notes.',
...
export function loadBookendStoryEvidenceCardPackBuildInputs() {
...
function renderSlip(slip, index) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home evidence slip</p>
      <h3>${escapeHtml(slip.title || `Evidence slip ${index + 1}`)}</h3>
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Bookend Story Evidence Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  const image = imagePath
    ? `<img src="${escapeHtml(imagePath)}" alt="${escapeHtml(world.title)} illustration">`
    : '<div class="image-placeholder" aria-hidden="true"></div>'
  const fields = [
    renderField('Story claim', card.storyClaimPrompt),
    renderField('First clue', card.firstCluePrompt),
    renderField('Second clue', card.secondCluePrompt),
    renderField('Because line', card.becauseLinePrompt),
    renderField('Evidence sentence', card.evidenceSentencePrompt),
    renderField('Bookend note', card.bookendNotePrompt),
  ].join('\n')

  return `
    <section class="pack-page evidence-card-page">
      <div class="page-kicker">Evidence Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.evidenceSkill)}</div>
...
export function renderBookendStoryEvidenceCardPackHtml(source, worlds, imageMap = new Map()) {
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .evidence-card-page { font-size: 7.3px; }
      ...
      .evidence-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .evidence-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    <section class="pack-page cover-page">
      <div>
        ...
        <p>Family-safe printable writing pages for adult-led, offline evidence practice.</p>
      </div>
    ...
    <section class="pack-page guide-page">
      ...
      <p class="footer-note">Use one story evidence card at a time. Keep every evidence sentence broad, invented, paper-only, and guided by an adult.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen evidence card worlds</h2>
      <p>Pick a world image, then name the story claim, first clue, second clue, because line, evidence sentence, and bookend note.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Evidence routines</p>
      <h2>Choose the story evidence routine</h2>
      <div class="routine-grid">${source.evidenceRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home evidence slips</p>
      <h2>Try one evidence path later</h2>
      <div class="slip-grid">${source.takeHomeEvidenceSlips.map(renderSlip).join('\n')}</div>
...
function prepareBuildDirectory(paths = bookendStoryEvidenceBuildPaths()) {
...
function copyPackAssets(source, paths = bookendStoryEvidenceBuildPaths(), options = {}) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
    const sourceImage = sourceImageForSlug(slug, options)
    if (!sourceImage) throw new Error(`Missing Bookend Story Evidence Card Pack source image for ${slug}`)
...
function writeReadme(source, paths = bookendStoryEvidenceBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = bookendStoryEvidenceBuildPaths()) {
  const entries = [
    {
      name: 'Bookend-Story-Evidence-Card-Pack.pdf',
    ...
    {
      name: 'source/bookend-story-evidence-card-pack.html',
...
function assertValidBuildInput(source, product, worlds, sourceFilesRoot = root) {
  const sourceErrors = validateBookendStoryEvidenceCardPackSource(
    source,
    product,
    worldAgeRecords(worlds),
  )
  const laneErrors = validateBookendStoryEvidenceCardPackSourceFiles(source, sourceFilesRoot)
  const errors = [...sourceErrors, ...laneErrors]
  if (errors.length > 0) {
    throw new Error(`Bookend Story Evidence Card Pack source is invalid:\n${errors.join('\n')}`)
  }
}

export async function buildBookendStoryEvidenceCardPack(options = {}) {
  const inputs = options.source
    ...
    : loadBookendStoryEvidenceCardPackBuildInputs()
  const { source, product, worlds } = inputs
  const paths = bookendStoryEvidenceBuildPaths(options.outputDir ?? options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildBookendStoryEvidenceCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
