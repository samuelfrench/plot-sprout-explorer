...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'expanding-file-story-scene-chain-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'expanding-file-story-scene-chain-card-pack')

function expandingFileStorySceneChainBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Expanding-File-Story-Scene-Chain-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'expanding-file-story-scene-chain-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'expanding-file-story-scene-chain-card-pack.html'),
...
function renderSlip(slip) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home scene slip</p>
      <h3>${escapeHtml(slip.title)}</h3>
      <p>${escapeHtml(slip.adultLine)}</p>
      <p>${escapeHtml(slip.childLine)}</p>
      <p>${escapeHtml(slip.nextPageLine)}</p>
    </article>`
}
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Expanding File Story Scene Chain Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Expanding File Story Scene Chain Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Start scene', card.startScenePrompt),
    renderField('Next scene', card.nextScenePrompt),
    renderField('Bridge detail', card.bridgeDetailPrompt),
    renderField('Change marker', card.changeMarkerPrompt),
    renderField('Return detail', card.returnDetailPrompt),
    renderField('File pocket note', card.filePocketPrompt),
  ].join('\n')

  return `
    <section class="pack-page scene-chain-card-page">
      <div class="page-kicker">Scene Chain Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.chainSkill)}</div>
...
export function renderExpandingFileStorySceneChainCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Expanding File Story Scene Chain Card source world slug: ${slug}`)
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .scene-chain-card-page { font-size: 7.48px; }
      ...
      .scene-chain-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .scene-chain-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    <section class="pack-page cover-page">
      <div>
        ...
        <p>Family-safe printable writing pages for adult-led, offline scene-chain practice.</p>
      </div>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Scene-chain coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one expanding file scene-chain card at a time. Keep every choice broad, invented, paper-only, and guided by an adult.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen scene-chain card worlds</h2>
      <p>Pick a world image, then connect one fictional page move with a pretend expanding file scene-chain card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Scene-chain routines</p>
      <h2>Choose the expanding file routine</h2>
      <div class="routine-grid">${source.sceneChainRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home scene slips</p>
      <h2>Try one scene chain later</h2>
      <div class="slip-grid">${source.takeHomeSceneSlips.map(renderSlip).join('\n')}</div>
...
export function loadExpandingFileStorySceneChainCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = expandingFileStorySceneChainBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = expandingFileStorySceneChainBuildPaths(), imageSources = null) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = expandingFileStorySceneChainBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = expandingFileStorySceneChainBuildPaths()) {
  const entries = [
    {
      name: 'Expanding-File-Story-Scene-Chain-Card-Pack.pdf',
    ...
    {
      name: 'source/expanding-file-story-scene-chain-card-pack.html',
...
export async function buildExpandingFileStorySceneChainCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadExpandingFileStorySceneChainCardPackBuildInputs()
  const paths = expandingFileStorySceneChainBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildExpandingFileStorySceneChainCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
