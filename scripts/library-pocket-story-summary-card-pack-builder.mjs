...
import {
  libraryPocketStorySummaryCardPackProductSlug,
...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'library-pocket-story-summary-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', libraryPocketStorySummaryCardPackProductSlug)
...
function libraryPocketStorySummaryBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Library-Pocket-Story-Summary-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'library-pocket-story-summary-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'library-pocket-story-summary-card-pack.html'),
...
function assembleSourceFromLanes() {
  ...
  if (cardLanes.length !== 3 || !tools) {
    throw new Error('Batch62 library pocket source lanes are incomplete.')
  }

  return {
    batchId: '2026-06-04-batch62',
    generatedAt: '2026-06-04',
    productSlug: libraryPocketStorySummaryCardPackProductSlug,
...
export function loadLibraryPocketStorySummaryCardPackBuildInputs() {
...
function renderSlip(slip, index) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home summary slip</p>
      <h3>${escapeHtml(slip.title || `Summary slip ${index + 1}`)}</h3>
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Library Pocket Story Summary Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  const image = imagePath
    ? `<img src="${escapeHtml(imagePath)}" alt="${escapeHtml(world.title)} illustration">`
    : '<div class="image-placeholder" aria-hidden="true"></div>'
  const fields = [
    renderField('Story start', card.storyStartPrompt),
    renderField('Main action', card.mainActionPrompt),
    renderField('Important change', card.importantChangePrompt),
    renderField('Ending result', card.endingResultPrompt),
    renderField('Keeper detail', card.keeperDetailPrompt),
    renderField('Summary sentence', card.summarySentencePrompt),
    renderField('Library pocket label', card.libraryPocketLabelPrompt),
  ].join('\n')

  return `
    <section class="pack-page summary-card-page">
      <div class="page-kicker">Summary Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.summarySkill)}</div>
...
export function renderLibraryPocketStorySummaryCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Library Pocket Story Summary Card source world slug: ${slug}`)
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .summary-card-page { font-size: 7.3px; }
      ...
      .summary-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .summary-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    <section class="pack-page cover-page">
      <div>
        ...
        <p>Family-safe printable writing pages for adult-led, offline summary practice.</p>
      </div>
    ...
    <section class="pack-page guide-page">
      ...
      <p class="footer-note">Use one story summary card at a time. Keep every summary line broad, invented, paper-only, and guided by an adult.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen summary card worlds</h2>
      <p>Pick a world image, then name the story start, main action, important change, ending result, keeper detail, summary sentence, and library pocket label.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Summary routines</p>
      <h2>Choose the story summary routine</h2>
      <div class="routine-grid">${source.summaryRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home summary slips</p>
      <h2>Try one summary later</h2>
      <div class="slip-grid">${source.takeHomeSummarySlips.map(renderSlip).join('\n')}</div>
...
function prepareBuildDirectory(paths = libraryPocketStorySummaryBuildPaths()) {
...
function copyPackAssets(source, paths = libraryPocketStorySummaryBuildPaths(), options = {}) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
    const sourceImage = sourceImageForSlug(slug, options)
    if (!sourceImage) throw new Error(`Missing Library Pocket Story Summary Card Pack source image for ${slug}`)
...
function writeReadme(source, paths = libraryPocketStorySummaryBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = libraryPocketStorySummaryBuildPaths()) {
  const entries = [
    {
      name: 'Library-Pocket-Story-Summary-Card-Pack.pdf',
    ...
    {
      name: 'source/library-pocket-story-summary-card-pack.html',
...
function assertValidBuildInput(source, product, worlds) {
  const sourceErrors = validateLibraryPocketStorySummaryCardPackSource(
    source,
    product,
  ...
  if (errors.length > 0) {
    throw new Error(`Library Pocket Story Summary Card Pack source is invalid:\n${errors.join('\n')}`)
  }
}

export async function buildLibraryPocketStorySummaryCardPack(options = {}) {
  const inputs = options.source
    ...
    : loadLibraryPocketStorySummaryCardPackBuildInputs()
  const { source, product, worlds } = inputs
  const paths = libraryPocketStorySummaryBuildPaths(options.outputDir ?? options.buildDir ?? buildDir)
  ...
  const html = renderLibraryPocketStorySummaryCardPackHtml(source, worlds, imageMap)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildLibraryPocketStorySummaryCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
