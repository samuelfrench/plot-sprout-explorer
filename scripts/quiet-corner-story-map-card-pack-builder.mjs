...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'quiet-corner-story-map-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'quiet-corner-story-map-card-pack')

function quietCornerMapBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Quiet-Corner-Story-Map-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'quiet-corner-story-map-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'quiet-corner-story-map-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Quiet Corner Story Map Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Quiet Corner Story Map Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Map front', card.mapFrontPrompt),
    renderField('Map back', card.mapBackPrompt),
    renderField('Story shape', card.storyShapePrompt),
    renderField('First line bridge', card.firstLineBridge),
    renderField('Revision nudge', card.revisionNudge),
    renderField('Quiet option', card.quietOptionLine),
  ].join('\n')

  return `
    <section class="pack-page story-map-card-page">
      <div class="page-kicker">Story Map Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.mapSkill)}</div>
...
export function renderQuietCornerStoryMapCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Quiet Corner Story Map Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-map-card-page { font-size: 7.52px; }
      ...
      .story-map-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-map-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story map coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Quiet-corner setup', source.adultGuide.quietCornerSetup)}
        ${renderGuideCard('Story map coaching', source.adultGuide.storyMapCoaching)}
    ...
    <section class="pack-page world-page">
      ...
      <h2>Sixteen story-map-card worlds</h2>
      <p>Pick a world image, then turn one invented map clue into a story start.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page format-page">
      <p class="page-kicker">Map routines</p>
      <h2>Choose the paper map rhythm</h2>
      <div class="format-grid">${source.mapRoutines.map(renderCardFormat).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home map slips</p>
      <h2>Finish one story clue later</h2>
      <div class="slip-grid">${source.takeHomeMapSlips.map(renderCardSlip).join('\n')}</div>
...
export function loadQuietCornerStoryMapCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = quietCornerMapBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = quietCornerMapBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = quietCornerMapBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Quiet-Corner-Story-Map-Card-Pack.pdf',
    '- source/quiet-corner-story-map-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = quietCornerMapBuildPaths()) {
  const entries = [
    {
      name: 'Quiet-Corner-Story-Map-Card-Pack.pdf',
    ...
    {
      name: 'source/quiet-corner-story-map-card-pack.html',
...
export async function buildQuietCornerStoryMapCardPack(options = {}) {
  const { source, product, worlds } = loadQuietCornerStoryMapCardPackBuildInputs()
  const paths = quietCornerMapBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderQuietCornerStoryMapCardPackHtml(source, worlds, imageMap)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildQuietCornerStoryMapCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
