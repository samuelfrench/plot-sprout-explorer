...
import {
  pageFlagStoryReasonChainCardPackProductSlug,
...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'page-flag-story-reason-chain-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', pageFlagStoryReasonChainCardPackProductSlug)
...
function pageFlagStoryReasonChainBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Page-Flag-Story-Reason-Chain-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'page-flag-story-reason-chain-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'page-flag-story-reason-chain-card-pack.html'),
...
function assembleSourceFromLanes() {
  ...
  if (cardLanes.length !== 3 || !tools) {
    throw new Error('Batch65 page flag reason chain source lanes are incomplete.')
  }

  return {
    batchId: '2026-06-04-batch65',
    generatedAt: '2026-06-04',
    productSlug: pageFlagStoryReasonChainCardPackProductSlug,
    ...
    sessionLength:
      '16 printable page flag story reason-chain cards plus adult guide tools, reason-chain routines, take-home reason slips, and optional adult prompts',
...
export function loadPageFlagStoryReasonChainCardPackBuildInputs() {
...
function renderSlip(slip, index) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home reason slip</p>
      <h3>${escapeHtml(slip.title || `Reason slip ${index + 1}`)}</h3>
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Page Flag Story Reason Chain Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  const image = imagePath
    ? `<img src="${escapeHtml(imagePath)}" alt="${escapeHtml(world.title)} illustration">`
    : '<div class="image-placeholder" aria-hidden="true"></div>'
  const fields = [
    renderField('Story idea', card.storyIdeaPrompt),
    renderField('First detail', card.firstDetailPrompt),
    renderField('First reason', card.firstReasonPrompt),
    renderField('Second detail', card.secondDetailPrompt),
    renderField('Because bridge', card.becauseBridgePrompt),
    renderField('Reason-chain sentence', card.reasonChainSentencePrompt),
    renderField('Page flag check', card.pageFlagCheckPrompt),
  ].join('\n')

  return `
    <section class="pack-page reason-chain-card-page">
      <div class="page-kicker">Reason Chain Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.reasonSkill)}</div>
...
export function renderPageFlagStoryReasonChainCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Page Flag Story Reason Chain Card source world slug: ${slug}`)
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .reason-chain-card-page { font-size: 7.3px; }
      ...
      .reason-chain-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .reason-chain-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    <section class="pack-page cover-page">
      <div>
        ...
        <p>Family-safe printable writing pages for adult-led, offline reason-chain practice.</p>
      </div>
    ...
    <section class="pack-page guide-page">
      ...
      <p class="footer-note">Use one story reason-chain card at a time. Keep every reason-chain sentence broad, invented, paper-only, and guided by an adult.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen reason chain card worlds</h2>
      <p>Pick a world image, then name the story idea, first detail, first reason, second detail, because bridge, reason-chain sentence, and page flag check.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Reason-chain routines</p>
      <h2>Choose the story reason-chain routine</h2>
      <div class="routine-grid">${source.reasonChainRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home reason slips</p>
      <h2>Try one reason chain later</h2>
      <div class="slip-grid">${source.takeHomeReasonSlips.map(renderSlip).join('\n')}</div>
...
function prepareBuildDirectory(paths = pageFlagStoryReasonChainBuildPaths()) {
...
function copyPackAssets(source, paths = pageFlagStoryReasonChainBuildPaths(), options = {}) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
    const sourceImage = sourceImageForSlug(slug, options)
    if (!sourceImage) throw new Error(`Missing Page Flag Story Reason Chain Card Pack source image for ${slug}`)
...
function writeReadme(source, paths = pageFlagStoryReasonChainBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = pageFlagStoryReasonChainBuildPaths()) {
  const entries = [
    {
      name: 'Page-Flag-Story-Reason-Chain-Card-Pack.pdf',
    ...
    {
      name: 'source/page-flag-story-reason-chain-card-pack.html',
...
function assertValidBuildInput(source, product, worlds, sourceFilesRoot = root) {
  const sourceErrors = validatePageFlagStoryReasonChainCardPackSource(
    source,
    product,
    worldAgeRecords(worlds),
  )
  const laneErrors = validatePageFlagStoryReasonChainCardPackSourceFiles(source, sourceFilesRoot)
  const errors = [...sourceErrors, ...laneErrors]
  if (errors.length > 0) {
    throw new Error(`Page Flag Story Reason Chain Card Pack source is invalid:\n${errors.join('\n')}`)
  }
}

export async function buildPageFlagStoryReasonChainCardPack(options = {}) {
  const inputs = options.source
    ...
    : loadPageFlagStoryReasonChainCardPackBuildInputs()
  const { source, product, worlds } = inputs
  const paths = pageFlagStoryReasonChainBuildPaths(options.outputDir ?? options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildPageFlagStoryReasonChainCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
