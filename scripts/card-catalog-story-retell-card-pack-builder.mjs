...
import {
  cardCatalogStoryRetellCardPackProductSlug,
...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'card-catalog-story-retell-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', cardCatalogStoryRetellCardPackProductSlug)
...
function cardCatalogStoryRetellBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Card-Catalog-Story-Retell-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'card-catalog-story-retell-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'card-catalog-story-retell-card-pack.html'),
...
function assembleSourceFromLanes() {
  ...
  if (cardLanes.length !== 3 || !tools) {
    throw new Error('Batch61 card catalog source lanes are incomplete.')
  }

  return {
    batchId: '2026-06-04-batch61',
    generatedAt: '2026-06-04',
    productSlug: cardCatalogStoryRetellCardPackProductSlug,
...
export function loadCardCatalogStoryRetellCardPackBuildInputs() {
...
function renderSlip(slip, index) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home retell slip</p>
      <h3>Retell slip ${index + 1}</h3>
      <p>${escapeHtml(slip)}</p>
    </article>`
}
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Card Catalog Story Retell Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  const image = imagePath
    ? `<img src="${escapeHtml(imagePath)}" alt="${escapeHtml(world.title)} illustration">`
    : '<div class="image-placeholder" aria-hidden="true"></div>'
  const fields = [
    renderField('Beginning snapshot', card.beginningSnapshotPrompt),
    renderField('Middle clue', card.middleCluePrompt),
    renderField('Turning choice', card.turningChoicePrompt),
    renderField('Ending answer', card.endingAnswerPrompt),
    renderField('Favorite detail', card.favoriteDetailPrompt),
    renderField('Next retell prompt', card.nextRetellPrompt),
    renderField('Card catalog label', card.cardCatalogLabelPrompt),
  ].join('\n')

  return `
    <section class="pack-page retell-card-page">
      <div class="page-kicker">Retell Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.retellSkill)}</div>
...
export function renderCardCatalogStoryRetellCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Card Catalog Story Retell Card source world slug: ${slug}`)
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .retell-card-page { font-size: 7.3px; }
      ...
      .retell-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .retell-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    <section class="pack-page cover-page">
      <div>
        ...
        <p>Family-safe printable writing pages for adult-led, offline retell practice.</p>
      </div>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Retell coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one story retell card at a time. Keep every retell line broad, invented, paper-only, and guided by an adult.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen retell card worlds</h2>
      <p>Pick a world image, then name the beginning, middle, turning choice, and ending in order.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Retell routines</p>
      <h2>Choose the story retell routine</h2>
      <div class="routine-grid">${source.retellRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home retell slips</p>
      <h2>Try one retell later</h2>
      <div class="slip-grid">${source.takeHomeRetellSlips.map(renderSlip).join('\n')}</div>
...
function prepareBuildDirectory(paths = cardCatalogStoryRetellBuildPaths()) {
...
function copyPackAssets(source, paths = cardCatalogStoryRetellBuildPaths(), options = {}) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
    const sourceImage = sourceImageForSlug(slug, options)
    if (!sourceImage) throw new Error(`Missing Card Catalog Story Retell Card Pack source image for ${slug}`)
...
function writeReadme(source, paths = cardCatalogStoryRetellBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = cardCatalogStoryRetellBuildPaths()) {
  const entries = [
    {
      name: 'Card-Catalog-Story-Retell-Card-Pack.pdf',
    ...
    {
      name: 'source/card-catalog-story-retell-card-pack.html',
...
function assertValidBuildInput(source, product, worlds) {
  const sourceErrors = validateCardCatalogStoryRetellCardPackSource(
    source,
    product,
  ...
  if (errors.length > 0) {
    throw new Error(`Card Catalog Story Retell Card Pack source is invalid:\n${errors.join('\n')}`)
  }
}

export async function buildCardCatalogStoryRetellCardPack(options = {}) {
  const inputs = options.source
    ...
    : loadCardCatalogStoryRetellCardPackBuildInputs()
  const { source, product, worlds } = inputs
  const paths = cardCatalogStoryRetellBuildPaths(options.outputDir ?? options.buildDir ?? buildDir)
  ...
  const html = renderCardCatalogStoryRetellCardPackHtml(source, worlds, imageMap)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildCardCatalogStoryRetellCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
