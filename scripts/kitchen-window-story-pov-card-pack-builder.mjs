...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'kitchen-window-story-pov-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'kitchen-window-story-pov-card-pack')

function kitchenWindowPovBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Kitchen-Window-Story-Point-of-View-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'kitchen-window-story-pov-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'kitchen-window-story-pov-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Kitchen Window POV Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Kitchen Window POV Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('First view', card.firstViewPrompt),
    renderField('Second view', card.secondViewPrompt),
    renderField('Near detail', card.nearDetailPrompt),
    renderField('Far detail', card.farDetailPrompt),
    renderField('Thought clue', card.thoughtCluePrompt),
    renderField('Revise view', card.reviseViewPrompt),
    renderField('Quiet option', card.quietOptionLine),
  ].join('\n')

  return `
    <section class="pack-page story-pov-card-page">
      <div class="page-kicker">POV Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.pointOfViewSkill)}</div>
...
export function renderKitchenWindowStoryPovCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Kitchen Window POV Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-pov-card-page { font-size: 7.52px; }
      ...
      .story-pov-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-pov-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Viewpoint coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper point-of-view setup', source.adultGuide.paperViewpointSetup)}
        ${renderGuideCard('Story viewpoint coaching', source.adultGuide.viewpointCoaching)}
      ...
      <p class="footer-note">Use one paper POV card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen POV-card worlds</h2>
      <p>Pick a world image, then write one fictional viewpoint sentence and one same-scene retell.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">POV routines</p>
      <h2>Choose the paper viewpoint rhythm</h2>
      <div class="routine-grid">${source.povRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home POV slips</p>
      <h2>Finish one story point-of-view later</h2>
      <div class="slip-grid">${source.takeHomePovSlips.map(renderSlip).join('\n')}</div>
...
export function loadKitchenWindowStoryPovCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = kitchenWindowPovBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = kitchenWindowPovBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = kitchenWindowPovBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Kitchen-Window-Story-Point-of-View-Card-Pack.pdf',
    '- source/kitchen-window-story-pov-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = kitchenWindowPovBuildPaths()) {
  const entries = [
    {
      name: 'Kitchen-Window-Story-Point-of-View-Card-Pack.pdf',
    ...
    {
      name: 'source/kitchen-window-story-pov-card-pack.html',
...
export async function buildKitchenWindowStoryPovCardPack(options = {}) {
  const { source, product, worlds } = loadKitchenWindowStoryPovCardPackBuildInputs()
  const paths = kitchenWindowPovBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildKitchenWindowStoryPovCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
