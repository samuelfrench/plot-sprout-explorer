...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'tabbed-folder-story-series-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'tabbed-folder-story-series-card-pack')

function tabbedFolderStorySeriesBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Tabbed-Folder-Story-Series-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'tabbed-folder-story-series-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'tabbed-folder-story-series-card-pack.html'),
...
function renderRoutine(routine) {
  return `
    <article class="small-card">
      ...
      <ol>${renderList(routine.adultSteps)}</ol>
      <p>${escapeHtml(routine.familyLine)}</p>
    </article>`
}
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Tabbed Folder Story Series Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Tabbed Folder Story Series Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Page one anchor', card.pageOneAnchorPrompt),
    renderField('Character return', card.characterReturnPrompt),
    renderField('Setting return', card.settingReturnPrompt),
    renderField('Clue carry', card.clueCarryPrompt),
    renderField('Page turn', card.pageTurnPrompt),
    renderField('Series wrap', card.seriesWrapPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-series-card-page">
      <div class="page-kicker">Story Series Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.seriesSkill)}</div>
...
export function renderTabbedFolderStorySeriesCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Tabbed Folder Story Series Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-series-card-page { font-size: 7.52px; }
      ...
      .story-series-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-series-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story-series coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one tabbed folder story-series card at a time. Keep every choice broad, invented, paper-only, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen story-series-card worlds</h2>
      <p>Pick a world image, then check one fictional story draft with a pretend tabbed folder story-series card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story-series routines</p>
      <h2>Choose the tabbed folder story series routine</h2>
      <div class="routine-grid">${source.seriesRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home story series slips</p>
      <h2>Try one story series later</h2>
      <div class="slip-grid">${source.takeHomeSeriesSlips.map(renderSlip).join('\n')}</div>
...
export function loadTabbedFolderStorySeriesCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = tabbedFolderStorySeriesBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = tabbedFolderStorySeriesBuildPaths(), imageSources = null) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = tabbedFolderStorySeriesBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = tabbedFolderStorySeriesBuildPaths()) {
  const entries = [
    {
      name: 'Tabbed-Folder-Story-Series-Card-Pack.pdf',
    ...
    {
      name: 'source/tabbed-folder-story-series-card-pack.html',
...
export async function buildTabbedFolderStorySeriesCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadTabbedFolderStorySeriesCardPackBuildInputs()
  const paths = tabbedFolderStorySeriesBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildTabbedFolderStorySeriesCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
