...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'paper-sleeve-story-sentence-variety-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'paper-sleeve-story-sentence-variety-card-pack')

function paperSleeveSentenceVarietyBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Paper-Sleeve-Story-Sentence-Variety-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'paper-sleeve-story-sentence-variety-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'paper-sleeve-story-sentence-variety-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Paper Sleeve Story Sentence Variety Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Paper Sleeve Story Sentence Variety Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Short sentence', card.shortSentencePrompt),
    renderField('Stretched sentence', card.longSentencePrompt),
    renderField('Question sentence', card.questionSentencePrompt),
    renderField('Starter swap', card.starterSwapPrompt),
    renderField('Sentence combine', card.sentenceCombinePrompt),
    renderField('Rhythm check', card.rhythmCheckPrompt),
    renderField('Final line', card.finalLinePrompt),
  ].join('\n')

  return `
    <section class="pack-page sentence-variety-card-page">
      <div class="page-kicker">Story Sentence Variety Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.sentenceVarietySkill)}</div>
...
export function renderPaperSleeveStorySentenceVarietyCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Paper Sleeve Story Sentence Variety Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .sentence-variety-card-page { font-size: 7.52px; }
      ...
      .sentence-variety-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .sentence-variety-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story sentence-variety coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one paper sleeve story sentence-variety card at a time. Keep every choice broad, invented, paper-only, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen sentence-variety-card worlds</h2>
      <p>Pick a world image, then revise one fictional sentence with a pretend paper sleeve story sentence-variety card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story sentence-variety routines</p>
      <h2>Choose the paper sleeve sentence-variety rhythm</h2>
      <div class="routine-grid">${source.sentenceVarietyRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home sentence slips</p>
      <h2>Try one sentence shape later</h2>
      <div class="slip-grid">${source.takeHomeSentenceSlips.map(renderSlip).join('\n')}</div>
...
export function loadPaperSleeveStorySentenceVarietyCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = paperSleeveSentenceVarietyBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = paperSleeveSentenceVarietyBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = paperSleeveSentenceVarietyBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Paper-Sleeve-Story-Sentence-Variety-Card-Pack.pdf',
    '- source/paper-sleeve-story-sentence-variety-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = paperSleeveSentenceVarietyBuildPaths()) {
  const entries = [
    {
      name: 'Paper-Sleeve-Story-Sentence-Variety-Card-Pack.pdf',
    ...
    {
      name: 'source/paper-sleeve-story-sentence-variety-card-pack.html',
...
export async function buildPaperSleeveStorySentenceVarietyCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadPaperSleeveStorySentenceVarietyCardPackBuildInputs()
  const paths = paperSleeveSentenceVarietyBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildPaperSleeveStorySentenceVarietyCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
