...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'lined-paper-story-paragraph-revision-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'lined-paper-story-paragraph-revision-card-pack')

function linedPaperParagraphRevisionBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Lined-Paper-Story-Paragraph-Revision-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'lined-paper-story-paragraph-revision-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'lined-paper-story-paragraph-revision-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Lined Paper Story Paragraph Revision Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Lined Paper Story Paragraph Revision Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Topic sentence', card.topicSentencePrompt),
    renderField('Detail order', card.detailOrderPrompt),
    renderField('Transition check', card.transitionCheckPrompt),
    renderField('Closing sentence', card.closingSentencePrompt),
    renderField('Repeated-word cut', card.repeatedWordCutPrompt),
    renderField('Final revised paragraph', card.finalRevisedParagraphPrompt),
  ].join('\n')

  return `
    <section class="pack-page paragraph-revision-card-page">
      <div class="page-kicker">Story Paragraph Revision Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.paragraphRevisionSkill)}</div>
...
export function renderLinedPaperStoryParagraphRevisionCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Lined Paper Story Paragraph Revision Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .paragraph-revision-card-page { font-size: 7.52px; }
      ...
      .paragraph-revision-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .paragraph-revision-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story paragraph-revision coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one lined-paper story paragraph-revision card at a time. Keep every choice broad, invented, paper-only, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen paragraph-revision-card worlds</h2>
      <p>Pick a world image, then revise one fictional paragraph with a pretend lined-paper story paragraph-revision card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story paragraph-revision routines</p>
      <h2>Choose the lined-paper paragraph-revision routine</h2>
      <div class="routine-grid">${source.paragraphRevisionRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home paragraph revision slips</p>
      <h2>Try one revised paragraph later</h2>
      <div class="slip-grid">${source.takeHomeParagraphRevisionSlips.map(renderSlip).join('\n')}</div>
...
export function loadLinedPaperStoryParagraphRevisionCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = linedPaperParagraphRevisionBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = linedPaperParagraphRevisionBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = linedPaperParagraphRevisionBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = linedPaperParagraphRevisionBuildPaths()) {
  const entries = [
    {
      name: 'Lined-Paper-Story-Paragraph-Revision-Card-Pack.pdf',
    ...
    {
      name: 'source/lined-paper-story-paragraph-revision-card-pack.html',
...
export async function buildLinedPaperStoryParagraphRevisionCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadLinedPaperStoryParagraphRevisionCardPackBuildInputs()
  const paths = linedPaperParagraphRevisionBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildLinedPaperStoryParagraphRevisionCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
