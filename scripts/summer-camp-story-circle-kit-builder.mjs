...
const buildDir = resolve(root, 'product-build', 'summer-camp-story-circle-kit')

function summerCampBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Summer-Camp-Story-Circle-Kit.pdf'),
    zipPath: resolve(targetBuildDir, 'summer-camp-story-circle-kit.zip'),
    htmlPath: resolve(sourceDir, 'summer-camp-story-circle-kit.html'),
...
function renderTakeHomeTrailCard(card) {
  return `
...
function renderCampActivityPage(activity, index, worlds, imageMap) {
  const world = worlds.get(activity.worldSlug)
  if (!world) throw new Error(`Unknown Summer Camp world slug: ${activity.worldSlug}`)
  const imagePath = imageMap.get(activity.worldSlug)
  if (!imagePath) throw new Error(`Missing Summer Camp copied image for ${activity.worldSlug}`)
  ...
  const sections = activity.pageSections
    .map(
      (section) => `
  ...
  return `
    <section class="pack-page camp-activity-page">
      <div class="page-kicker">Circle ${index + 1} | Ages ${escapeHtml(activity.ageBand)}</div>
      <h2>${escapeHtml(activity.title)}</h2>
      ${imageBlock}
      <p class="kid-direction">${escapeHtml(activity.kidDirection)}</p>
      <p class="adult-note">${escapeHtml(activity.counselorPrompt)}</p>
      <div class="quest-meta">
        <span>${escapeHtml(activity.circleSkill)}</span>
        <span>${escapeHtml(activity.sessionFit)}</span>
      </div>
      <p><strong>Counselor setup:</strong> ${escapeHtml(activity.counselorSetup)}</p>
      <div class="worksheet-sections">${sections}</div>
      <p class="wrap-line">${escapeHtml(activity.groupTwistLine)}</p>
      <p class="wrap-line">${escapeHtml(activity.wrapUpLine)}</p>
      <p class="extension-line">${escapeHtml(activity.quietOptionLine)}</p>
    </section>`
}

export function renderSummerCampStoryCircleKitHtml(source, worlds, imageMap = new Map()) {
  const worldIntros = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Summer Camp source world slug: ${slug}`)
      return renderWorldIntro(world, imageMap.get(slug))
    })
    .join('\n')
  const activityPages = source.activities
    .map((activity, index) => renderCampActivityPage(activity, index, worlds, imageMap))
    .join('\n')
...
<html lang="en">
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      <p class="page-kicker">Counselor setup</p>
      <h2>Run the story circles</h2>
      <div class="setup-grid">
        <div>
          <h3>Before camp</h3>
          <ol>${renderList(source.counselorGuide.beforeCamp)}</ol>
        </div>
        <div>
          <h3>Setup</h3>
          <ol>${renderList(source.counselorGuide.setup)}</ol>
        </div>
        <div>
          <h3>Running circles</h3>
          <ol>${renderList(source.counselorGuide.runningCircle)}</ol>
        </div>
        <div>
          <h3>Quiet options</h3>
          <ol>${renderList(source.counselorGuide.quietOptions)}</ol>
        </div>
        <div>
          <h3>No-data use</h3>
          <ol>${renderList(source.counselorGuide.noDataUse)}</ol>
        </div>
      </div>
      <h3>Image-backed world menu</h3>
      <div class="world-grid">${worldIntros}</div>
      <p class="footer-note">Use these pages offline for adult-guided summer camp. Keep finished pages in folders or send them home.</p>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Counselor tools</p>
      <h2>Circle formats and optional share prompts</h2>
      <div class="routine-grid">${source.circleFormats.map(renderRoutine).join('\n')}</div>
      <h3>Optional share prompts</h3>
      <ul class="share-grid">${renderList(source.optionalSharePrompts)}</ul>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Counselor tools</p>
      <h2>Take-home trail cards</h2>
      <div class="slip-grid">${source.takeHomeTrailCards.map(renderTakeHomeTrailCard).join('\n')}</div>
    </section>
${activityPages}
  </body>
</html>
...
export function loadSummerCampStoryCircleBuildInputs() {
  ...
  for (const activity of source.activities) {
    if (!worlds.has(activity.worldSlug)) throw new Error(`Missing activity world record for ${activity.worldSlug}`)
    if (!imageMap.has(activity.worldSlug)) throw new Error(`Missing local copied image for activity world ${activity.worldSlug}`)
  }
  return {
    source,
    product,
...
function prepareBuildDirectory(paths = summerCampBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = summerCampBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = summerCampBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Summer-Camp-Story-Circle-Kit.pdf',
    '- source/summer-camp-story-circle-kit.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = summerCampBuildPaths()) {
  const entries = [
    {
      name: 'Summer-Camp-Story-Circle-Kit.pdf',
    ...
    {
      name: 'source/summer-camp-story-circle-kit.html',
...
export async function buildSummerCampStoryCircleKit(options = {}) {
  const { source, product, worlds } = loadSummerCampStoryCircleBuildInputs()
  const paths = summerCampBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderSummerCampStoryCircleKitHtml(source, worlds, imageMap)
