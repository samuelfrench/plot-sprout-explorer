...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'blanket-fort-story-dialogue-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'blanket-fort-story-dialogue-card-pack')

function blanketFortDialogueBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Blanket-Fort-Story-Dialogue-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'blanket-fort-story-dialogue-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'blanket-fort-story-dialogue-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Blanket Fort Dialogue Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Blanket Fort Dialogue Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Speaker one', card.speakerOnePrompt),
    renderField('Speaker two', card.speakerTwoPrompt),
    renderField('Dialogue tag', card.dialogueTagPrompt),
    renderField('Feeling clue', card.feelingCluePrompt),
    renderField('Reply prompt', card.replyPrompt),
    renderField('Revise line', card.reviseLinePrompt),
    renderField('Quiet option', card.quietOptionLine),
  ].join('\n')

  return `
    <section class="pack-page story-dialogue-card-page">
      <div class="page-kicker">Dialogue Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.dialogueSkill)}</div>
...
export function renderBlanketFortStoryDialogueCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Blanket Fort Dialogue Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-dialogue-card-page { font-size: 7.52px; }
      ...
      .story-dialogue-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-dialogue-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Dialogue move coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper dialogue setup', source.adultGuide.paperDialogueSetup)}
        ${renderGuideCard('Story dialogue coaching', source.adultGuide.dialogueCoaching)}
      ...
      <p class="footer-note">Use one paper dialogue card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen dialogue-card worlds</h2>
      <p>Pick a world image, then write one fictional speaker line and one matching reply.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Dialogue routines</p>
      <h2>Choose the paper dialogue rhythm</h2>
      <div class="routine-grid">${source.dialogueRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home dialogue slips</p>
      <h2>Finish one story dialogue later</h2>
      <div class="slip-grid">${source.takeHomeDialogueSlips.map(renderSlip).join('\n')}</div>
...
export function loadBlanketFortStoryDialogueCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = blanketFortDialogueBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = blanketFortDialogueBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = blanketFortDialogueBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Blanket-Fort-Story-Dialogue-Card-Pack.pdf',
    '- source/blanket-fort-story-dialogue-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = blanketFortDialogueBuildPaths()) {
  const entries = [
    {
      name: 'Blanket-Fort-Story-Dialogue-Card-Pack.pdf',
    ...
    {
      name: 'source/blanket-fort-story-dialogue-card-pack.html',
...
export async function buildBlanketFortStoryDialogueCardPack(options = {}) {
  const { source, product, worlds } = loadBlanketFortStoryDialogueCardPackBuildInputs()
  const paths = blanketFortDialogueBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildBlanketFortStoryDialogueCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
