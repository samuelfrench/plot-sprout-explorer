...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'notebook-margin-story-revision-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'notebook-margin-story-revision-card-pack')

function notebookMarginRevisionBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Notebook-Margin-Story-Revision-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'notebook-margin-story-revision-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'notebook-margin-story-revision-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Notebook Margin Story Revision Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Notebook Margin Story Revision Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Margin prompt', card.marginPrompt),
    renderField('Draft line', card.draftLinePrompt),
    renderField('Revision move', card.revisionMovePrompt),
    renderField('New line', card.newLinePrompt),
    renderField('Check back', card.checkBackPrompt),
    renderField('Quiet option', card.quietOptionLine),
  ].join('\n')

  return `
    <section class="pack-page story-revision-card-page">
      <div class="page-kicker">Story Revision Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.revisionSkill)}</div>
...
export function renderNotebookMarginStoryRevisionCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Notebook Margin Story Revision Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-revision-card-page { font-size: 7.52px; }
      ...
      .story-revision-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-revision-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Revision move coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper revision setup', source.adultGuide.paperRevisionSetup)}
        ${renderGuideCard('Story revision coaching', source.adultGuide.revisionCoaching)}
      ...
      <p class="footer-note">Use one paper revision card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen story-revision-card worlds</h2>
      <p>Pick a world image, then turn one invented notebook-margin choice into a story start.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Revision routines</p>
      <h2>Choose the paper revision rhythm</h2>
      <div class="routine-grid">${source.revisionRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home revision slips</p>
      <h2>Finish one story revision later</h2>
      <div class="slip-grid">${source.takeHomeRevisionSlips.map(renderSlip).join('\n')}</div>
...
export function loadNotebookMarginStoryRevisionCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = notebookMarginRevisionBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = notebookMarginRevisionBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = notebookMarginRevisionBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Notebook-Margin-Story-Revision-Card-Pack.pdf',
    '- source/notebook-margin-story-revision-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = notebookMarginRevisionBuildPaths()) {
  const entries = [
    {
      name: 'Notebook-Margin-Story-Revision-Card-Pack.pdf',
    ...
    {
      name: 'source/notebook-margin-story-revision-card-pack.html',
...
export async function buildNotebookMarginStoryRevisionCardPack(options = {}) {
  const { source, product, worlds } = loadNotebookMarginStoryRevisionCardPackBuildInputs()
  const paths = notebookMarginRevisionBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderNotebookMarginStoryRevisionCardPackHtml(source, worlds, imageMap)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildNotebookMarginStoryRevisionCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
