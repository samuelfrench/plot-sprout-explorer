...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'accordion-folder-story-arc-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'accordion-folder-story-arc-card-pack')

function accordionFolderStoryArcBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Accordion-Folder-Story-Arc-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'accordion-folder-story-arc-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'accordion-folder-story-arc-card-pack.html'),
...
function renderSlip(slip) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home story arc slip</p>
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Accordion Folder Story Arc Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Accordion Folder Story Arc Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Beginning', card.beginningPrompt),
    renderField('Middle change', card.middleChangePrompt),
    renderField('Choice bridge', card.choiceBridgePrompt),
    renderField('Consequence', card.consequencePrompt),
    renderField('Ending return', card.endingReturnPrompt),
    renderField('Arc folder note', card.arcFolderPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-arc-card-page">
      <div class="page-kicker">Story Arc Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.arcSkill)}</div>
...
export function renderAccordionFolderStoryArcCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Accordion Folder Story Arc Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-arc-card-page { font-size: 7.52px; }
      ...
      .story-arc-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-arc-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story-arc coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one accordion folder story-arc card at a time. Keep every choice broad, invented, paper-only, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen story-arc card worlds</h2>
      <p>Pick a world image, then shape one fictional story draft with a pretend accordion folder story-arc card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story-arc routines</p>
      <h2>Choose the accordion folder story arc routine</h2>
      <div class="routine-grid">${source.arcRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home story arc slips</p>
      <h2>Try one story arc later</h2>
      <div class="slip-grid">${source.takeHomeArcSlips.map(renderSlip).join('\n')}</div>
...
export function loadAccordionFolderStoryArcCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = accordionFolderStoryArcBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = accordionFolderStoryArcBuildPaths(), imageSources = null) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = accordionFolderStoryArcBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = accordionFolderStoryArcBuildPaths()) {
  const entries = [
    {
      name: 'Accordion-Folder-Story-Arc-Card-Pack.pdf',
    ...
    {
      name: 'source/accordion-folder-story-arc-card-pack.html',
...
export async function buildAccordionFolderStoryArcCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadAccordionFolderStoryArcCardPackBuildInputs()
  const paths = accordionFolderStoryArcBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildAccordionFolderStoryArcCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
