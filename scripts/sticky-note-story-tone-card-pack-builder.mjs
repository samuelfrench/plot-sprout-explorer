...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'sticky-note-story-tone-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'sticky-note-story-tone-card-pack')

function stickyNoteToneBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Sticky-Note-Story-Tone-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'sticky-note-story-tone-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'sticky-note-story-tone-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Sticky Note Story Tone Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Sticky Note Story Tone Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Neutral line', card.neutralLinePrompt),
    renderField('Tone choice', card.toneChoicePrompt),
    renderField('Word choice', card.wordChoicePrompt),
    renderField('Object signal', card.objectSignalPrompt),
    renderField('Place cue', card.placeCuePrompt),
    renderField('Gesture tone', card.gestureTonePrompt),
    renderField('Sentence frame', card.sentenceFramePrompt),
    renderField('Revise story tone', card.reviseTonePrompt),
  ].join('\n')

  return `
    <section class="pack-page story-tone-card-page">
      <div class="page-kicker">Story Tone Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.toneSkill)}</div>
...
export function renderStickyNoteStoryToneCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Sticky Note Story Tone Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-tone-card-page { font-size: 7.52px; }
      ...
      .story-tone-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-tone-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story tone coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one sticky note story tone card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen story tone-card worlds</h2>
      <p>Pick a world image, then start one fictional story with a pretend sticky note story tone card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story tone routines</p>
      <h2>Choose the sticky note story tone rhythm</h2>
      <div class="routine-grid">${source.toneRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home tone slips</p>
      <h2>Start one story tone line later</h2>
      <div class="slip-grid">${source.takeHomeToneSlips.map(renderSlip).join('\n')}</div>
...
export function loadStickyNoteStoryToneCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = stickyNoteToneBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = stickyNoteToneBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = stickyNoteToneBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Sticky-Note-Story-Tone-Card-Pack.pdf',
    '- source/sticky-note-story-tone-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = stickyNoteToneBuildPaths()) {
  const entries = [
    {
      name: 'Sticky-Note-Story-Tone-Card-Pack.pdf',
    ...
    {
      name: 'source/sticky-note-story-tone-card-pack.html',
...
export async function buildStickyNoteStoryToneCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadStickyNoteStoryToneCardPackBuildInputs()
  const paths = stickyNoteToneBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildStickyNoteStoryToneCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
