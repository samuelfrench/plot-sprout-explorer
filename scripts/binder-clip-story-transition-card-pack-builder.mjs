...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'binder-clip-story-transition-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'binder-clip-story-transition-card-pack')

function binderClipTransitionBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Binder-Clip-Story-Transition-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'binder-clip-story-transition-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'binder-clip-story-transition-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Binder Clip Transition Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Binder Clip Transition Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Before moment', card.beforeMomentPrompt),
    renderField('After moment', card.afterMomentPrompt),
    renderField('Bridge word', card.bridgeWordPrompt),
    renderField('Character move', card.characterMovePrompt),
    renderField('Place shift', card.placeShiftPrompt),
    renderField('Object carry-over', card.objectCarryPrompt),
    renderField('Transition question', card.transitionQuestionPrompt),
    renderField('Revise transition', card.reviseTransitionPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-transition-card-page">
      <div class="page-kicker">Transition Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.transitionSkill)}</div>
...
export function renderBinderClipStoryTransitionCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Binder Clip Transition Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-transition-card-page { font-size: 7.52px; }
      ...
      .story-transition-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-transition-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Transition coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one binder clip transition card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen transition-card worlds</h2>
      <p>Pick a world image, then start one fictional story with a pretend binder clip transition card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Transition routines</p>
      <h2>Choose the binder clip transition rhythm</h2>
      <div class="routine-grid">${source.transitionRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home transition slips</p>
      <h2>Start one story transition later</h2>
      <div class="slip-grid">${source.takeHomeTransitionSlips.map(renderSlip).join('\n')}</div>
...
export function loadBinderClipStoryTransitionCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = binderClipTransitionBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = binderClipTransitionBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = binderClipTransitionBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Binder-Clip-Story-Transition-Card-Pack.pdf',
    '- source/binder-clip-story-transition-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = binderClipTransitionBuildPaths()) {
  const entries = [
    {
      name: 'Binder-Clip-Story-Transition-Card-Pack.pdf',
    ...
    {
      name: 'source/binder-clip-story-transition-card-pack.html',
...
export async function buildBinderClipStoryTransitionCardPack(options = {}) {
  const { source, product, worlds } = loadBinderClipStoryTransitionCardPackBuildInputs()
  const paths = binderClipTransitionBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildBinderClipStoryTransitionCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
