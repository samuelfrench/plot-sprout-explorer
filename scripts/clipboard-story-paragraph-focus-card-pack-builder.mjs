...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'clipboard-story-paragraph-focus-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'clipboard-story-paragraph-focus-card-pack')

function clipboardParagraphFocusBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Clipboard-Story-Paragraph-Focus-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'clipboard-story-paragraph-focus-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'clipboard-story-paragraph-focus-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Clipboard Story Paragraph Focus Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Clipboard Story Paragraph Focus Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Main idea', card.mainIdeaPrompt),
    renderField('Detail one', card.detailOnePrompt),
    renderField('Detail two', card.detailTwoPrompt),
    renderField('Detail order', card.detailOrderPrompt),
    renderField('Linking sentence', card.linkingSentencePrompt),
    renderField('Cut extra', card.cutExtraPrompt),
    renderField('Final paragraph', card.finalParagraphPrompt),
  ].join('\n')

  return `
    <section class="pack-page paragraph-focus-card-page">
      <div class="page-kicker">Story Paragraph Focus Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.paragraphFocusSkill)}</div>
...
export function renderClipboardStoryParagraphFocusCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Clipboard Story Paragraph Focus Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .paragraph-focus-card-page { font-size: 7.52px; }
      ...
      .paragraph-focus-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .paragraph-focus-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story paragraph-focus coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one clipboard story paragraph-focus card at a time. Keep every choice broad, invented, paper-only, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen paragraph-focus-card worlds</h2>
      <p>Pick a world image, then shape one fictional paragraph with a pretend clipboard story paragraph-focus card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story paragraph-focus routines</p>
      <h2>Choose the clipboard paragraph-focus routine</h2>
      <div class="routine-grid">${source.paragraphFocusRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home paragraph slips</p>
      <h2>Try one focused paragraph later</h2>
      <div class="slip-grid">${source.takeHomeParagraphSlips.map(renderSlip).join('\n')}</div>
...
export function loadClipboardStoryParagraphFocusCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = clipboardParagraphFocusBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = clipboardParagraphFocusBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = clipboardParagraphFocusBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = clipboardParagraphFocusBuildPaths()) {
  const entries = [
    {
      name: 'Clipboard-Story-Paragraph-Focus-Card-Pack.pdf',
    ...
    {
      name: 'source/clipboard-story-paragraph-focus-card-pack.html',
...
export async function buildClipboardStoryParagraphFocusCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadClipboardStoryParagraphFocusCardPackBuildInputs()
  const paths = clipboardParagraphFocusBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildClipboardStoryParagraphFocusCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
