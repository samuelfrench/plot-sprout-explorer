...
const buildDir = resolve(root, 'product-build', 'grandparent-story-visit-kit')

function grandparentVisitBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Grandparent-Story-Visit-Kit.pdf'),
    zipPath: resolve(targetBuildDir, 'grandparent-story-visit-kit.zip'),
    htmlPath: resolve(sourceDir, 'grandparent-story-visit-kit.html'),
...
function renderTakeHomePostcard(card) {
  return `
...
function renderVisitQuest(visitQuest, index, worlds, imageMap) {
  const world = worlds.get(visitQuest.worldSlug)
  if (!world) throw new Error(`Unknown Grandparent Story Visit world slug: ${visitQuest.worldSlug}`)
  const imagePath = imageMap.get(visitQuest.worldSlug)
  if (!imagePath) throw new Error(`Missing Grandparent Story Visit copied image for ${visitQuest.worldSlug}`)
  ...
  const sections = visitQuest.pageSections
    .map(
      (section) => `
  ...
  return `
    <section class="pack-page visit-quest-page">
      <div class="page-kicker">Visit quest ${index + 1} | Ages ${escapeHtml(visitQuest.ageBand)}</div>
      <h2>${escapeHtml(visitQuest.title)}</h2>
      ${imageBlock}
      <p class="kid-direction">${escapeHtml(visitQuest.kidDirection)}</p>
      <p class="adult-note">${escapeHtml(visitQuest.hostPrompt)}</p>
      <div class="quest-meta">
        <span>${escapeHtml(visitQuest.visitSkill)}</span>
        <span>${escapeHtml(visitQuest.visitFit)}</span>
      </div>
      <p><strong>Adult setup:</strong> ${escapeHtml(visitQuest.adultSetup)}</p>
      <div class="worksheet-sections">${sections}</div>
      <p class="extension-line">${escapeHtml(visitQuest.quietOption)}</p>
      <p class="take-home-line">${escapeHtml(visitQuest.takeHomeLine)}</p>
    </section>`
}

export function renderGrandparentStoryVisitKitHtml(source, worlds, imageMap = new Map()) {
  const worldIntros = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Grandparent Story Visit source world slug: ${slug}`)
      return renderWorldIntro(world, imageMap.get(slug))
    })
    .join('\n')
  const visitQuests = source.visitQuests
    .map((quest, index) => renderVisitQuest(quest, index, worlds, imageMap))
    .join('\n')
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .visit-quest-page figure {
        float: right;
        width: 1.58in;
        margin: 0 0 0.055in 0.11in;
      }
      .visit-quest-page figure img {
        width: 100%;
        aspect-ratio: 1.25 / 1;
        object-fit: cover;
      }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Start the visit story</h2>
      <div class="guide-grid">
        <article class="guide-card"><h3>Visit setup</h3><ul>${renderList(source.hostGuide.visitSetup)}</ul></article>
        <article class="guide-card"><h3>Story hosting</h3><ul>${renderList(source.hostGuide.storyHosting)}</ul></article>
        ...
        <article class="guide-card"><h3>Take-home handoff</h3><ul>${renderList(source.hostGuide.takeHomeHandoff)}</ul></article>
        <article class="guide-card"><h3>Pack reset</h3><ul>${renderList(source.hostGuide.packReset)}</ul></article>
      </div>
      <p class="footer-note">Use one quest at a time. Keep every detail invented, printable, and offline.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Image-backed visit quest menu</h2>
      <p>Pick one quest at a time. Each page uses an invented world image so visit-table details become fiction without online sharing.</p>
      <div class="world-grid">${worldIntros}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Visit tools</p>
      <h2>Repeatable visit formats and optional family-share prompts</h2>
    ...
    <section class="pack-page slip-page">
      ...
      <h2>Take-home story postcards</h2>
      <div class="slip-grid">${source.takeHomePostcards.map(renderTakeHomePostcard).join('\n')}</div>
    </section>
${visitQuests}
  </body>
</html>
...
export function loadGrandparentStoryVisitKitBuildInputs() {
  ...
  for (const quest of source.visitQuests) {
    if (!worlds.has(quest.worldSlug)) throw new Error(`Missing visit quest world record for ${quest.worldSlug}`)
    if (!imageMap.has(quest.worldSlug)) throw new Error(`Missing local copied image for visit quest world ${quest.worldSlug}`)
  }
  return {
    source,
    product,
...
function prepareBuildDirectory(paths = grandparentVisitBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = grandparentVisitBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = grandparentVisitBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Grandparent-Story-Visit-Kit.pdf',
    '- source/grandparent-story-visit-kit.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = grandparentVisitBuildPaths()) {
  const entries = [
    {
      name: 'Grandparent-Story-Visit-Kit.pdf',
    ...
    {
      name: 'source/grandparent-story-visit-kit.html',
...
export async function buildGrandparentStoryVisitKit(options = {}) {
  const { source, product, worlds } = loadGrandparentStoryVisitKitBuildInputs()
  const paths = grandparentVisitBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderGrandparentStoryVisitKitHtml(source, worlds, imageMap)
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildGrandparentStoryVisitKit().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
