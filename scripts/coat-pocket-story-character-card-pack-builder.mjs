...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'coat-pocket-story-character-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'coat-pocket-story-character-card-pack')

function coatPocketCharacterBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Coat-Pocket-Story-Character-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'coat-pocket-story-character-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'coat-pocket-story-character-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Coat Pocket Character Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Coat Pocket Character Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Character name', card.characterNamePrompt),
    renderField('Pocket item', card.pocketItemPrompt),
    renderField('Want clue', card.wantPrompt),
    renderField('Worry clue', card.worryPrompt),
    renderField('Action clue', card.actionPrompt),
    renderField('Voice choice', card.voicePrompt),
    renderField('Revise character', card.reviseCharacterPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-character-card-page">
      <div class="page-kicker">Character Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.characterSkill)}</div>
...
export function renderCoatPocketStoryCharacterCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Coat Pocket Character Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-character-card-page { font-size: 7.52px; }
      ...
      .story-character-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-character-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Character coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper character setup', source.adultGuide.paperCharacterSetup)}
        ${renderGuideCard('Story character coaching', source.adultGuide.characterCoaching)}
      ...
      <p class="footer-note">Use one paper character card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen character-card worlds</h2>
      <p>Pick a world image, then create one fictional character from a pretend paper pocket item.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">character routines</p>
      <h2>Choose the paper character rhythm</h2>
      <div class="routine-grid">${source.characterRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home character slips</p>
      <h2>Finish one story character later</h2>
      <div class="slip-grid">${source.takeHomeCharacterSlips.map(renderSlip).join('\n')}</div>
...
export function loadCoatPocketStoryCharacterCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = coatPocketCharacterBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = coatPocketCharacterBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = coatPocketCharacterBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Coat-Pocket-Story-Character-Card-Pack.pdf',
    '- source/coat-pocket-story-character-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = coatPocketCharacterBuildPaths()) {
  const entries = [
    {
      name: 'Coat-Pocket-Story-Character-Card-Pack.pdf',
    ...
    {
      name: 'source/coat-pocket-story-character-card-pack.html',
...
export async function buildCoatPocketStoryCharacterCardPack(options = {}) {
  const { source, product, worlds } = loadCoatPocketStoryCharacterCardPackBuildInputs()
  const paths = coatPocketCharacterBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildCoatPocketStoryCharacterCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
