...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'index-card-story-show-not-tell-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'index-card-story-show-not-tell-card-pack')

function indexCardShowNotTellBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Index-Card-Story-Show-Not-Tell-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'index-card-story-show-not-tell-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'index-card-story-show-not-tell-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Index Card Show-Not-Tell Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Index Card Show-Not-Tell Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Plain line', card.plainLinePrompt),
    renderField('Visible clue', card.visibleCluePrompt),
    renderField('Object action', card.objectActionPrompt),
    renderField('Place signal', card.placeSignalPrompt),
    renderField('Character gesture', card.characterGesturePrompt),
    renderField('Sound or texture', card.soundOrTexturePrompt),
    renderField('Sentence frame', card.sentenceFramePrompt),
    renderField('Revise show-not-tell', card.reviseShowPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-show-not-tell-card-page">
      <div class="page-kicker">Show-Not-Tell Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.showSkill)}</div>
...
export function renderIndexCardStoryShowNotTellCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Index Card Show-Not-Tell Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-show-not-tell-card-page { font-size: 7.52px; }
      ...
      .story-show-not-tell-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-show-not-tell-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Show-not-tell coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one index card show-not-tell card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen show-not-tell-card worlds</h2>
      <p>Pick a world image, then start one fictional story with a pretend index card show-not-tell card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Show-not-tell routines</p>
      <h2>Choose the index card show-not-tell rhythm</h2>
      <div class="routine-grid">${source.showRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home show slips</p>
      <h2>Start one story shown line later</h2>
      <div class="slip-grid">${source.takeHomeShowSlips.map(renderSlip).join('\n')}</div>
...
export function loadIndexCardStoryShowNotTellCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = indexCardShowNotTellBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = indexCardShowNotTellBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = indexCardShowNotTellBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Index-Card-Story-Show-Not-Tell-Card-Pack.pdf',
    '- source/index-card-story-show-not-tell-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = indexCardShowNotTellBuildPaths()) {
  const entries = [
    {
      name: 'Index-Card-Story-Show-Not-Tell-Card-Pack.pdf',
    ...
    {
      name: 'source/index-card-story-show-not-tell-card-pack.html',
...
export async function buildIndexCardStoryShowNotTellCardPack(options = {}) {
  const { source, product, worlds } = loadIndexCardStoryShowNotTellCardPackBuildInputs()
  const paths = indexCardShowNotTellBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildIndexCardStoryShowNotTellCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
