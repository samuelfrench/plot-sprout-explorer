...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'pencil-case-story-switch-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'pencil-case-story-switch-card-pack')

function pencilCaseSwitchBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Pencil-Case-Story-Switch-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'pencil-case-story-switch-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'pencil-case-story-switch-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Pencil Case Story Switch Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Pencil Case Story Switch Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Switch front', card.switchFrontPrompt),
    renderField('Switch back', card.switchBackPrompt),
    renderField('Story switch', card.storySwitchPrompt),
    renderField('First line switch', card.firstLineSwitch),
    renderField('Revision nudge', card.revisionNudge),
    renderField('Quiet option', card.quietOptionLine),
  ].join('\n')

  return `
    <section class="pack-page story-switch-card-page">
      <div class="page-kicker">Story Switch Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.switchSkill)}</div>
...
export function renderPencilCaseStorySwitchCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Pencil Case Story Switch Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-switch-card-page { font-size: 7.52px; }
      ...
      .story-switch-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-switch-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story switch coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper switch setup', source.adultGuide.paperSwitchSetup)}
        ${renderGuideCard('Story switch coaching', source.adultGuide.storySwitchCoaching)}
      ...
      <p class="footer-note">Use one paper switch card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen story-switch-card worlds</h2>
      <p>Pick a world image, then turn one invented pencil-case choice into a story start.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Switch routines</p>
      <h2>Choose the paper switch rhythm</h2>
      <div class="routine-grid">${source.switchRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home switch slips</p>
      <h2>Finish one story switch later</h2>
      <div class="slip-grid">${source.takeHomeSwitchSlips.map(renderSlip).join('\n')}</div>
...
export function loadPencilCaseStorySwitchCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = pencilCaseSwitchBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = pencilCaseSwitchBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = pencilCaseSwitchBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Pencil-Case-Story-Switch-Card-Pack.pdf',
    '- source/pencil-case-story-switch-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = pencilCaseSwitchBuildPaths()) {
  const entries = [
    {
      name: 'Pencil-Case-Story-Switch-Card-Pack.pdf',
    ...
    {
      name: 'source/pencil-case-story-switch-card-pack.html',
...
export async function buildPencilCaseStorySwitchCardPack(options = {}) {
  const { source, product, worlds } = loadPencilCaseStorySwitchCardPackBuildInputs()
  const paths = pencilCaseSwitchBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderPencilCaseStorySwitchCardPackHtml(source, worlds, imageMap)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildPencilCaseStorySwitchCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
