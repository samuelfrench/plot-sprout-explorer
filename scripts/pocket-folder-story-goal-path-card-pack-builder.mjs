...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'pocket-folder-story-goal-path-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'pocket-folder-story-goal-path-card-pack')

function pocketFolderStoryGoalPathBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Pocket-Folder-Story-Goal-Path-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'pocket-folder-story-goal-path-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'pocket-folder-story-goal-path-card-pack.html'),
...
function renderSlip(slip, index) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home goal slip</p>
      <h3>Goal slip ${index + 1}</h3>
      <p>${escapeHtml(slip)}</p>
    </article>`
}
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Pocket Folder Story Goal Path Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Pocket Folder Story Goal Path Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Want', card.wantPrompt),
    renderField('Snag', card.snagPrompt),
    renderField('First try', card.firstTryPrompt),
    renderField('Rethink', card.rethinkPrompt),
    renderField('Finish note', card.finishNotePrompt),
    renderField('Pocket label', card.pocketLabelPrompt),
  ].join('\n')

  return `
    <section class="pack-page goal-path-card-page">
      <div class="page-kicker">Goal Path Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.goalSkill)}</div>
...
export function renderPocketFolderStoryGoalPathCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Pocket Folder Story Goal Path Card source world slug: ${slug}`)
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .goal-path-card-page { font-size: 7.48px; }
      ...
      .goal-path-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .goal-path-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    <section class="pack-page cover-page">
      <div>
        ...
        <p>Family-safe printable writing pages for adult-led, offline goal path practice.</p>
      </div>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Goal path coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one pocket folder goal path card at a time. Keep every choice broad, invented, paper-only, and guided by an adult.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen goal path card worlds</h2>
      <p>Pick a world image, then connect one fictional want, snag, first try, rethink, finish note, and pocket label.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Goal path routines</p>
      <h2>Choose the pocket folder routine</h2>
      <div class="routine-grid">${source.goalPathRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home goal slips</p>
      <h2>Try one goal path later</h2>
      <div class="slip-grid">${source.takeHomeGoalSlips.map(renderSlip).join('\n')}</div>
...
export function loadPocketFolderStoryGoalPathCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = pocketFolderStoryGoalPathBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = pocketFolderStoryGoalPathBuildPaths(), imageSources = null) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = pocketFolderStoryGoalPathBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = pocketFolderStoryGoalPathBuildPaths()) {
  const entries = [
    {
      name: 'Pocket-Folder-Story-Goal-Path-Card-Pack.pdf',
    ...
    {
      name: 'source/pocket-folder-story-goal-path-card-pack.html',
...
export async function buildPocketFolderStoryGoalPathCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadPocketFolderStoryGoalPathCardPackBuildInputs()
  const paths = pocketFolderStoryGoalPathBuildPaths(options.buildDir ?? buildDir)
  ...
  const manifest = buildProductArtifactManifest(source, {
    ...
    fulfillmentNote: 'Static artifact bundle: PDF plus source HTML and local image assets; sales path stays pending on the site.',
  })
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildPocketFolderStoryGoalPathCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
