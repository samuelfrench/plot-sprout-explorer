...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'composition-notebook-story-draft-checklist-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'composition-notebook-story-draft-checklist-card-pack')

function compositionNotebookDraftChecklistBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Composition-Notebook-Story-Draft-Checklist-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'composition-notebook-story-draft-checklist-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'composition-notebook-story-draft-checklist-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Composition Notebook Story Draft Checklist Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Composition Notebook Story Draft Checklist Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Character check', card.characterCheckPrompt),
    renderField('Setting check', card.settingCheckPrompt),
    renderField('Sequence check', card.sequenceCheckPrompt),
    renderField('Detail check', card.detailCheckPrompt),
    renderField('Sentence check', card.sentenceCheckPrompt),
    renderField('Final draft checklist', card.finalDraftChecklistPrompt),
  ].join('\n')

  return `
    <section class="pack-page draft-checklist-card-page">
      <div class="page-kicker">Story Draft Checklist Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.draftChecklistSkill)}</div>
...
export function renderCompositionNotebookStoryDraftChecklistCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Composition Notebook Story Draft Checklist Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .draft-checklist-card-page { font-size: 7.52px; }
      ...
      .draft-checklist-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .draft-checklist-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story draft-checklist coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one composition notebook story draft-checklist card at a time. Keep every choice broad, invented, paper-only, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen draft-checklist-card worlds</h2>
      <p>Pick a world image, then check one fictional story draft with a pretend composition notebook draft-checklist card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story draft-checklist routines</p>
      <h2>Choose the composition notebook draft-checklist routine</h2>
      <div class="routine-grid">${source.draftChecklistRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home draft checklist slips</p>
      <h2>Try one draft checklist later</h2>
      <div class="slip-grid">${source.takeHomeDraftChecklistSlips.map(renderSlip).join('\n')}</div>
...
export function loadCompositionNotebookStoryDraftChecklistCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = compositionNotebookDraftChecklistBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = compositionNotebookDraftChecklistBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = compositionNotebookDraftChecklistBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = compositionNotebookDraftChecklistBuildPaths()) {
  const entries = [
    {
      name: 'Composition-Notebook-Story-Draft-Checklist-Card-Pack.pdf',
    ...
    {
      name: 'source/composition-notebook-story-draft-checklist-card-pack.html',
...
export async function buildCompositionNotebookStoryDraftChecklistCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadCompositionNotebookStoryDraftChecklistCardPackBuildInputs()
  const paths = compositionNotebookDraftChecklistBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildCompositionNotebookStoryDraftChecklistCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
