...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'spiral-notebook-story-final-copy-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'spiral-notebook-story-final-copy-card-pack')

function spiralNotebookFinalCopyBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Spiral-Notebook-Story-Final-Copy-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'spiral-notebook-story-final-copy-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'spiral-notebook-story-final-copy-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Spiral Notebook Story Final Copy Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Spiral Notebook Story Final Copy Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Opening copy', card.openingCopyPrompt),
    renderField('Neat copy', card.neatCopyPrompt),
    renderField('Detail transfer', card.detailTransferPrompt),
    renderField('Sentence boundary', card.sentenceBoundaryPrompt),
    renderField('Dialogue copy', card.dialogueCopyPrompt),
    renderField('Final-copy check', card.finalCopyCheckPrompt),
  ].join('\n')

  return `
    <section class="pack-page final-copy-card-page">
      <div class="page-kicker">Story Final Copy Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.finalCopySkill)}</div>
...
export function renderSpiralNotebookStoryFinalCopyCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Spiral Notebook Story Final Copy Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .final-copy-card-page { font-size: 7.52px; }
      ...
      .final-copy-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .final-copy-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story final-copy coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one spiral notebook story final-copy card at a time. Keep every choice broad, invented, paper-only, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen final-copy-card worlds</h2>
      <p>Pick a world image, then check one fictional story draft with a pretend spiral notebook final-copy card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story final-copy routines</p>
      <h2>Choose the spiral notebook final-copy routine</h2>
      <div class="routine-grid">${source.finalCopyRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home final copy slips</p>
      <h2>Try one final copy later</h2>
      <div class="slip-grid">${source.takeHomeFinalCopySlips.map(renderSlip).join('\n')}</div>
...
export function loadSpiralNotebookStoryFinalCopyCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = spiralNotebookFinalCopyBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = spiralNotebookFinalCopyBuildPaths(), imageSources = null) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = spiralNotebookFinalCopyBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = spiralNotebookFinalCopyBuildPaths()) {
  const entries = [
    {
      name: 'Spiral-Notebook-Story-Final-Copy-Card-Pack.pdf',
    ...
    {
      name: 'source/spiral-notebook-story-final-copy-card-pack.html',
...
export async function buildSpiralNotebookStoryFinalCopyCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadSpiralNotebookStoryFinalCopyCardPackBuildInputs()
  const paths = spiralNotebookFinalCopyBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildSpiralNotebookStoryFinalCopyCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
