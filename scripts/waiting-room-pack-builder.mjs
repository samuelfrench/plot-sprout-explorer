...
const buildDir = resolve(root, 'product-build', 'waiting-room-story-quest-pack')

function waitingRoomBuildPaths(targetBuildDir = buildDir) {
  ...
  return {
    ...
    pdfPath: resolve(targetBuildDir, 'Waiting-Room-Story-Quest-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'waiting-room-story-quest-pack.zip'),
    htmlPath: resolve(targetSourceDir, 'waiting-room-story-quest-pack.html'),
...
function renderQuestPage(quest, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Waiting Room world slug: ${quest.worldSlug}`)
  ...
  return `
    <section class="pack-page quest-page">
      <div class="page-kicker">Waiting quest ${index + 1} | ${escapeHtml(quest.waitingMode)}</div>
      ...
      <div class="quest-meta">
        ...
        <span>${escapeHtml(quest.waitingUse)}</span>
      </div>
...
export function renderWaitingRoomPackHtml(source, worlds, imageMap = new Map()) {
  const worldIntros = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Waiting Room world slug: ${slug}`)
...
<html lang="en">
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Run the quiet waiting pack</h2>
      <div class="setup-grid">
        <div>
          <h3>Before you wait</h3>
          <ol>${renderList(source.setupGuide.beforeYouWait)}</ol>
        </div>
        <div>
          <h3>Restaurant table</h3>
          <ol>${renderList(source.setupGuide.restaurantTable)}</ol>
        </div>
        <div>
          <h3>Appointment lobby</h3>
          <ol>${renderList(source.setupGuide.appointmentLobby)}</ol>
        </div>
        <div>
          <h3>Sibling event</h3>
          <ol>${renderList(source.setupGuide.siblingEvent)}</ol>
        </div>
        <div>
          <h3>Pickup line</h3>
          <ol>${renderList(source.setupGuide.pickupLine)}</ol>
        </div>
      </div>
      <h3>World menu</h3>
      <div class="world-grid">${worldIntros}</div>
      <p class="footer-note">Use these pages offline for adult-guided waiting writing with no accounts, no uploads, and no public sharing.</p>
    </section>
    <section class="pack-page share-page">
      <p class="page-kicker">Waiting tools</p>
      <h2>Routines and share cards</h2>
      <div class="routine-grid">${source.waitingRoutines.map(renderRoutine).join('\n')}</div>
...
export function loadWaitingRoomBuildInputs() {
...
function prepareBuildDirectory(paths = waitingRoomBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = waitingRoomBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = waitingRoomBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Waiting-Room-Story-Quest-Pack.pdf',
    '- source/waiting-room-story-quest-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = waitingRoomBuildPaths()) {
  const entries = [
    {
      name: 'Waiting-Room-Story-Quest-Pack.pdf',
    ...
    {
      name: 'source/waiting-room-story-quest-pack.html',
...
export async function buildWaitingRoomPack(options = {}) {
  const { source, product, worlds } = loadWaitingRoomBuildInputs()
  const paths = waitingRoomBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderWaitingRoomPackHtml(source, worlds, imageMap)
