...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'hanging-file-story-decision-point-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'hanging-file-story-decision-point-card-pack')

function hangingFileStoryDecisionPointBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Hanging-File-Story-Decision-Point-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'hanging-file-story-decision-point-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'hanging-file-story-decision-point-card-pack.html'),
...
function renderSlip(slip, index) {
  return `
    <article class="card-slip">
      <p class="card-kicker">Take-home decision slip</p>
      <h3>Decision slip ${index + 1}</h3>
      <p>${escapeHtml(slip)}</p>
    </article>`
}
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Hanging File Story Decision Point Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Hanging File Story Decision Point Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Choice', card.choicePrompt),
    renderField('Path one', card.pathOnePrompt),
    renderField('Path two', card.pathTwoPrompt),
    renderField('Compare clue', card.compareCluePrompt),
    renderField('Chosen path', card.chosenPathPrompt),
    renderField('Consequence note', card.consequenceNotePrompt),
    renderField('File label', card.fileLabelPrompt),
  ].join('\n')

  return `
    <section class="pack-page decision-card-page">
      <div class="page-kicker">Decision Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.decisionSkill)}</div>
...
export function renderHangingFileStoryDecisionPointCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Hanging File Story Decision Point Card source world slug: ${slug}`)
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .decision-card-page { font-size: 7.3px; }
      ...
      .decision-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .decision-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    <section class="pack-page cover-page">
      <div>
        ...
        <p>Family-safe printable writing pages for adult-led, offline decision practice.</p>
      </div>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Decision point coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one hanging file card at a time. Keep every choice broad, invented, paper-only, and guided by an adult.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen decision card worlds</h2>
      <p>Pick a world image, then compare two fictional paths and choose one consequence note.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Decision point routines</p>
      <h2>Choose the hanging file routine</h2>
      <div class="routine-grid">${source.decisionPointRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home decision slips</p>
      <h2>Try one decision later</h2>
      <div class="slip-grid">${source.takeHomeDecisionSlips.map(renderSlip).join('\n')}</div>
...
export function loadHangingFileStoryDecisionPointCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = hangingFileStoryDecisionPointBuildPaths()) {
...
function copyPackAssets(source, paths = hangingFileStoryDecisionPointBuildPaths(), options = {}) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
    const sourceImage = sourceImageForSlug(slug, options)
    if (!sourceImage) throw new Error(`Missing Hanging File Story Decision Point Card Pack source image for ${slug}`)
...
function writeReadme(source, paths = hangingFileStoryDecisionPointBuildPaths()) {
  const text = [
    source.title,
    '',
...
function zipEntries(paths = hangingFileStoryDecisionPointBuildPaths()) {
  const entries = [
    {
      name: 'Hanging-File-Story-Decision-Point-Card-Pack.pdf',
    ...
    {
      name: 'source/hanging-file-story-decision-point-card-pack.html',
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildHangingFileStoryDecisionPointCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
