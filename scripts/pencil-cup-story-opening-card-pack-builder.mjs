...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'pencil-cup-story-opening-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'pencil-cup-story-opening-card-pack')

function pencilCupOpeningBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Pencil-Cup-Story-Opening-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'pencil-cup-story-opening-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'pencil-cup-story-opening-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Pencil Cup Opening Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Pencil Cup Opening Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('First line', card.firstLinePrompt),
    renderField('Character arrival', card.characterPrompt),
    renderField('Place doorway', card.placePrompt),
    renderField('Object invitation', card.objectPrompt),
    renderField('Opening question', card.questionPrompt),
    renderField('First next move', card.nextMovePrompt),
    renderField('Revise opening', card.reviseOpeningPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-opening-card-page">
      <div class="page-kicker">Opening Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.openingSkill)}</div>
...
export function renderPencilCupStoryOpeningCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Pencil Cup Opening Card source world slug: ${slug}`)
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-opening-card-page { font-size: 7.52px; }
      ...
      .story-opening-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-opening-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <div class="guide-grid">
        ...
        ${renderGuideCard('Pencil cup opening setup', source.adultGuide.pencilCupOpeningSetup)}
        ${renderGuideCard('Story opening coaching', source.adultGuide.openingCoaching)}
      ...
      <p class="footer-note">Use one pencil cup opening card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen opening-card worlds</h2>
      <p>Pick a world image, then start one fictional story with a pretend pencil cup opening card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Opening routines</p>
      <h2>Choose the pencil cup opening rhythm</h2>
      <div class="routine-grid">${source.openingRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home opening slips</p>
      <h2>Start one story opening later</h2>
      <div class="slip-grid">${source.takeHomeOpeningSlips.map(renderSlip).join('\n')}</div>
...
export function loadPencilCupStoryOpeningCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = pencilCupOpeningBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = pencilCupOpeningBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = pencilCupOpeningBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Pencil-Cup-Story-Opening-Card-Pack.pdf',
    '- source/pencil-cup-story-opening-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = pencilCupOpeningBuildPaths()) {
  const entries = [
    {
      name: 'Pencil-Cup-Story-Opening-Card-Pack.pdf',
    ...
    {
      name: 'source/pencil-cup-story-opening-card-pack.html',
...
export async function buildPencilCupStoryOpeningCardPack(options = {}) {
  const { source, product, worlds } = loadPencilCupStoryOpeningCardPackBuildInputs()
  const paths = pencilCupOpeningBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildPencilCupStoryOpeningCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
