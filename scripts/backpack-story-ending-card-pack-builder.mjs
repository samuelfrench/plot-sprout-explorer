...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'backpack-story-ending-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'backpack-story-ending-card-pack')

function backpackEndingBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Backpack-Story-Ending-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'backpack-story-ending-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'backpack-story-ending-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Backpack Ending Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Backpack Ending Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Ending choice', card.endingChoicePrompt),
    renderField('Final feeling', card.feelingPrompt),
    renderField('Object return', card.objectReturnPrompt),
    renderField('Echo line', card.echoPrompt),
    renderField('Final line', card.finalLinePrompt),
    renderField('Revise ending', card.reviseEndingPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-ending-card-page">
      <div class="page-kicker">Ending Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.endingSkill)}</div>
...
export function renderBackpackStoryEndingCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Backpack Ending Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-ending-card-page { font-size: 7.52px; }
      ...
      .story-ending-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-ending-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Ending coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Backpack ending setup', source.adultGuide.backpackEndingSetup)}
        ${renderGuideCard('Story ending coaching', source.adultGuide.endingCoaching)}
      ...
      <p class="footer-note">Use one backpack ending card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen ending-card worlds</h2>
      <p>Pick a world image, then close one fictional story with a pretend backpack ending card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">ending routines</p>
      <h2>Choose the backpack ending rhythm</h2>
      <div class="routine-grid">${source.endingRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home ending slips</p>
      <h2>Finish one story ending later</h2>
      <div class="slip-grid">${source.takeHomeEndingSlips.map(renderSlip).join('\n')}</div>
...
export function loadBackpackStoryEndingCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = backpackEndingBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = backpackEndingBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = backpackEndingBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Backpack-Story-Ending-Card-Pack.pdf',
    '- source/backpack-story-ending-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = backpackEndingBuildPaths()) {
  const entries = [
    {
      name: 'Backpack-Story-Ending-Card-Pack.pdf',
    ...
    {
      name: 'source/backpack-story-ending-card-pack.html',
...
export async function buildBackpackStoryEndingCardPack(options = {}) {
  const { source, product, worlds } = loadBackpackStoryEndingCardPackBuildInputs()
  const paths = backpackEndingBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildBackpackStoryEndingCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
