...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'porch-light-story-signal-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'porch-light-story-signal-card-pack')

function porchLightSignalBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Porch-Light-Story-Signal-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'porch-light-story-signal-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'porch-light-story-signal-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Porch Light Story Signal Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Porch Light Story Signal Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Signal front', card.signalFrontPrompt),
    renderField('Signal back', card.signalBackPrompt),
    renderField('Story signal', card.storySignalPrompt),
    renderField('First line signal', card.firstLineSignal),
    renderField('Revision nudge', card.revisionNudge),
    renderField('Quiet option', card.quietOptionLine),
  ].join('\n')

  return `
    <section class="pack-page story-signal-card-page">
      <div class="page-kicker">Story Signal Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.signalSkill)}</div>
...
export function renderPorchLightStorySignalCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Porch Light Story Signal Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-signal-card-page { font-size: 7.52px; }
      ...
      .story-signal-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-signal-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story signal coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper signal setup', source.adultGuide.paperSignalSetup)}
        ${renderGuideCard('Story signal coaching', source.adultGuide.storySignalCoaching)}
      ...
      <p class="footer-note">Use one paper signal card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen story-signal-card worlds</h2>
      <p>Pick a world image, then turn one invented signal cue into a story start.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page format-page">
      <p class="page-kicker">Signal routines</p>
      <h2>Choose the paper signal rhythm</h2>
      <div class="format-grid">${source.signalRoutines.map(renderCardFormat).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home signal slips</p>
      <h2>Finish one story signal later</h2>
      <div class="slip-grid">${source.takeHomeSignalSlips.map(renderCardSlip).join('\n')}</div>
...
export function loadPorchLightStorySignalCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = porchLightSignalBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = porchLightSignalBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = porchLightSignalBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Porch-Light-Story-Signal-Card-Pack.pdf',
    '- source/porch-light-story-signal-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = porchLightSignalBuildPaths()) {
  const entries = [
    {
      name: 'Porch-Light-Story-Signal-Card-Pack.pdf',
    ...
    {
      name: 'source/porch-light-story-signal-card-pack.html',
...
export async function buildPorchLightStorySignalCardPack(options = {}) {
  const { source, product, worlds } = loadPorchLightStorySignalCardPackBuildInputs()
  const paths = porchLightSignalBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderPorchLightStorySignalCardPackHtml(source, worlds, imageMap)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildPorchLightStorySignalCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
