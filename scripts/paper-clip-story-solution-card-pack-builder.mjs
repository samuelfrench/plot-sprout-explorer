...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'paper-clip-story-solution-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'paper-clip-story-solution-card-pack')

function paperClipSolutionBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Paper-Clip-Story-Solution-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'paper-clip-story-solution-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'paper-clip-story-solution-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Paper Clip Solution Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Paper Clip Solution Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Solution step', card.solutionStepPrompt),
    renderField('Character choice', card.characterChoicePrompt),
    renderField('Place clue', card.placeCluePrompt),
    renderField('Object use', card.objectUsePrompt),
    renderField('Solution question', card.solutionQuestionPrompt),
    renderField('First step', card.firstStepPrompt),
    renderField('Revise solution', card.reviseSolutionPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-solution-card-page">
      <div class="page-kicker">Solution Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.solutionSkill)}</div>
...
export function renderPaperClipStorySolutionCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Paper Clip Solution Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-solution-card-page { font-size: 7.52px; }
      ...
      .story-solution-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-solution-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Solution coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper clip solution setup', source.adultGuide.paperClipSolutionSetup)}
        ${renderGuideCard('Story solution coaching', source.adultGuide.solutionCoaching)}
      ...
      <p class="footer-note">Use one paper clip solution card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen solution-card worlds</h2>
      <p>Pick a world image, then start one fictional story with a pretend paper clip solution card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Solution routines</p>
      <h2>Choose the paper clip solution rhythm</h2>
      <div class="routine-grid">${source.solutionRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home solution slips</p>
      <h2>Start one story solution later</h2>
      <div class="slip-grid">${source.takeHomeSolutionSlips.map(renderSlip).join('\n')}</div>
...
export function loadPaperClipStorySolutionCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = paperClipSolutionBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = paperClipSolutionBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = paperClipSolutionBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Paper-Clip-Story-Solution-Card-Pack.pdf',
    '- source/paper-clip-story-solution-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = paperClipSolutionBuildPaths()) {
  const entries = [
    {
      name: 'Paper-Clip-Story-Solution-Card-Pack.pdf',
    ...
    {
      name: 'source/paper-clip-story-solution-card-pack.html',
...
export async function buildPaperClipStorySolutionCardPack(options = {}) {
  const { source, product, worlds } = loadPaperClipStorySolutionCardPackBuildInputs()
  const paths = paperClipSolutionBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildPaperClipStorySolutionCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
