...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'washi-tape-story-word-choice-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'washi-tape-story-word-choice-card-pack')

function washiTapeWordChoiceBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Washi-Tape-Story-Word-Choice-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'washi-tape-story-word-choice-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'washi-tape-story-word-choice-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Washi Tape Story Word Choice Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Washi Tape Story Word Choice Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Plain word', card.plainWordPrompt),
    renderField('Precise noun', card.preciseNounPrompt),
    renderField('Clear verb', card.clearVerbPrompt),
    renderField('Useful describer', card.describerPrompt),
    renderField('Sentence swap', card.sentenceSwapPrompt),
    renderField('Sound and shape check', card.soundShapePrompt),
    renderField('Final line', card.finalLinePrompt),
  ].join('\n')

  return `
    <section class="pack-page word-choice-card-page">
      <div class="page-kicker">Story Word Choice Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.wordChoiceSkill)}</div>
...
export function renderWashiTapeStoryWordChoiceCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Washi Tape Story Word Choice Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .word-choice-card-page { font-size: 7.52px; }
      ...
      .word-choice-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .word-choice-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Story word-choice coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one washi tape story word-choice card at a time. Keep every choice broad, invented, paper-only, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen word-choice-card worlds</h2>
      <p>Pick a world image, then revise one fictional sentence with a pretend washi tape story word-choice card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Story word-choice routines</p>
      <h2>Choose the washi tape word-choice rhythm</h2>
      <div class="routine-grid">${source.wordChoiceRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home word slips</p>
      <h2>Swap one word later</h2>
      <div class="slip-grid">${source.takeHomeWordSlips.map(renderSlip).join('\n')}</div>
...
export function loadWashiTapeStoryWordChoiceCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = washiTapeWordChoiceBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = washiTapeWordChoiceBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = washiTapeWordChoiceBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Washi-Tape-Story-Word-Choice-Card-Pack.pdf',
    '- source/washi-tape-story-word-choice-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = washiTapeWordChoiceBuildPaths()) {
  const entries = [
    {
      name: 'Washi-Tape-Story-Word-Choice-Card-Pack.pdf',
    ...
    {
      name: 'source/washi-tape-story-word-choice-card-pack.html',
...
export async function buildWashiTapeStoryWordChoiceCardPack(options = {}) {
  const {
    source,
    product,
    worlds,
  } = options.source ? options : loadWashiTapeStoryWordChoiceCardPackBuildInputs()
  const paths = washiTapeWordChoiceBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildWashiTapeStoryWordChoiceCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
