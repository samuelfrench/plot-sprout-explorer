...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'paper-tray-story-setting-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'paper-tray-story-setting-card-pack')

function paperTraySettingBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Paper-Tray-Story-Setting-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'paper-tray-story-setting-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'paper-tray-story-setting-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Paper Tray Setting Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Paper Tray Setting Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Setting name', card.settingNamePrompt),
    renderField('Tray label', card.trayLabelPrompt),
    renderField('Mood clue', card.moodPrompt),
    renderField('Sensory clue', card.sensoryPrompt),
    renderField('Map edge', card.mapEdgePrompt),
    renderField('Object anchor', card.objectAnchorPrompt),
    renderField('Revise setting', card.reviseSettingPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-setting-card-page">
      <div class="page-kicker">Setting Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.settingSkill)}</div>
...
export function renderPaperTrayStorySettingCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Paper Tray Setting Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-setting-card-page { font-size: 7.52px; }
      ...
      .story-setting-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-setting-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Setting coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Paper setting setup', source.adultGuide.paperSettingSetup)}
        ${renderGuideCard('Story setting coaching', source.adultGuide.settingCoaching)}
      ...
      <p class="footer-note">Use one paper setting card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen setting-card worlds</h2>
      <p>Pick a world image, then create one fictional setting from a pretend paper tray label.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">setting routines</p>
      <h2>Choose the paper setting rhythm</h2>
      <div class="routine-grid">${source.settingRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home setting slips</p>
      <h2>Finish one story setting later</h2>
      <div class="slip-grid">${source.takeHomeSettingSlips.map(renderSlip).join('\n')}</div>
...
export function loadPaperTrayStorySettingCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = paperTraySettingBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = paperTraySettingBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = paperTraySettingBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Paper-Tray-Story-Setting-Card-Pack.pdf',
    '- source/paper-tray-story-setting-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = paperTraySettingBuildPaths()) {
  const entries = [
    {
      name: 'Paper-Tray-Story-Setting-Card-Pack.pdf',
    ...
    {
      name: 'source/paper-tray-story-setting-card-pack.html',
...
export async function buildPaperTrayStorySettingCardPack(options = {}) {
  const { source, product, worlds } = loadPaperTrayStorySettingCardPackBuildInputs()
  const paths = paperTraySettingBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildPaperTrayStorySettingCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
