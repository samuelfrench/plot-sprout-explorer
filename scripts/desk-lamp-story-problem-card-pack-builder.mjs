...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'desk-lamp-story-problem-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'desk-lamp-story-problem-card-pack')

function deskLampProblemBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Desk-Lamp-Story-Problem-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'desk-lamp-story-problem-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'desk-lamp-story-problem-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Desk Lamp Problem Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Desk Lamp Problem Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Problem spot', card.problemSpotPrompt),
    renderField('Character need', card.characterNeedPrompt),
    renderField('Place pressure', card.placePressurePrompt),
    renderField('Object trouble', card.objectTroublePrompt),
    renderField('Problem question', card.questionPrompt),
    renderField('First try', card.firstTryPrompt),
    renderField('Revise problem', card.reviseProblemPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-problem-card-page">
      <div class="page-kicker">Problem Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.problemSkill)}</div>
...
export function renderDeskLampStoryProblemCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Desk Lamp Problem Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-problem-card-page { font-size: 7.52px; }
      ...
      .story-problem-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-problem-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Problem coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard('Before the session', source.adultGuide.beforeSession)}
        ${renderGuideCard('Desk lamp problem setup', source.adultGuide.deskLampProblemSetup)}
        ${renderGuideCard('Story problem coaching', source.adultGuide.problemCoaching)}
      ...
      <p class="footer-note">Use one desk lamp problem card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen problem-card worlds</h2>
      <p>Pick a world image, then start one fictional story with a pretend desk lamp problem card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Problem routines</p>
      <h2>Choose the desk lamp problem rhythm</h2>
      <div class="routine-grid">${source.problemRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home problem slips</p>
      <h2>Start one story problem later</h2>
      <div class="slip-grid">${source.takeHomeProblemSlips.map(renderSlip).join('\n')}</div>
...
export function loadDeskLampStoryProblemCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = deskLampProblemBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = deskLampProblemBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = deskLampProblemBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Desk-Lamp-Story-Problem-Card-Pack.pdf',
    '- source/desk-lamp-story-problem-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = deskLampProblemBuildPaths()) {
  const entries = [
    {
      name: 'Desk-Lamp-Story-Problem-Card-Pack.pdf',
    ...
    {
      name: 'source/desk-lamp-story-problem-card-pack.html',
...
export async function buildDeskLampStoryProblemCardPack(options = {}) {
  const { source, product, worlds } = loadDeskLampStoryProblemCardPackBuildInputs()
  const paths = deskLampProblemBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildDeskLampStoryProblemCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
