...
const buildDir = resolve(root, 'product-build', 'road-trip-story-quest-pack')

function roadTripBuildPaths(targetBuildDir = buildDir) {
  ...
  return {
    ...
    pdfPath: resolve(targetBuildDir, 'Road-Trip-Story-Quest-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'road-trip-story-quest-pack.zip'),
    htmlPath: resolve(targetSourceDir, 'road-trip-story-quest-pack.html'),
...
function renderQuestPage(quest, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Road Trip world slug: ${quest.worldSlug}`)
  ...
  return `
    <section class="pack-page quest-page">
      <div class="page-kicker">Travel quest ${index + 1} | ${escapeHtml(quest.travelMode)}</div>
      ...
      <div class="quest-meta">
        ...
        <span>${escapeHtml(quest.travelUse)}</span>
      </div>
...
export function renderRoadTripKitHtml(source, worlds, imageMap = new Map()) {
  const worldIntros = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Road Trip world slug: ${slug}`)
...
<html lang="en">
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Run the travel pack</h2>
      <div class="setup-grid">
        <div>
          <h3>Before you go</h3>
          <ol>${renderList(source.setupGuide.beforeYouGo)}</ol>
        </div>
        <div>
          <h3>In the car</h3>
          <ol>${renderList(source.setupGuide.inTheCar)}</ol>
        </div>
        <div>
          <h3>Rest stop or hotel</h3>
          <ol>${renderList(source.setupGuide.restStopHotel)}</ol>
        </div>
        <div>
          <h3>Visit day</h3>
          <ol>${renderList(source.setupGuide.visitDay)}</ol>
        </div>
      </div>
      <h3>World menu</h3>
      <div class="world-grid">${worldIntros}</div>
      <p class="footer-note">Use these pages offline for adult-guided travel writing with no accounts, no uploads, and no public sharing.</p>
    </section>
    <section class="pack-page share-page">
      <p class="page-kicker">Travel tools</p>
      <h2>Routines and share cards</h2>
      <div class="routine-grid">${source.travelRoutines.map(renderRoutine).join('\n')}</div>
...
export function loadRoadTripBuildInputs() {
...
function prepareBuildDirectory(paths = roadTripBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = roadTripBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = roadTripBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Road-Trip-Story-Quest-Pack.pdf',
    '- source/road-trip-story-quest-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = roadTripBuildPaths()) {
  const entries = [
    {
      name: 'Road-Trip-Story-Quest-Pack.pdf',
    ...
    {
      name: 'source/road-trip-story-quest-pack.html',
...
export async function buildRoadTripKit(options = {}) {
  const { source, product, worlds } = loadRoadTripBuildInputs()
  const paths = roadTripBuildPaths(options.buildDir ?? buildDir)
  const recordRoot = options.recordRoot ?? root

  prepareBuildDirectory(paths)
  const imageMap = copyPackAssets(source, paths)
  const html = renderRoadTripKitHtml(source, worlds, imageMap)
