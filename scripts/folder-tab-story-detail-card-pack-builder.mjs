...
const sourcePath = resolve(root, 'content', 'product-artifacts', 'folder-tab-story-detail-card-pack.json')
const productsPath = resolve(root, 'content', 'products', 'batch5-products.json')
const worldsDir = resolve(root, 'content', 'worlds')
const buildDir = resolve(root, 'product-build', 'folder-tab-story-detail-card-pack')

function folderTabDetailBuildPaths(targetBuildDir = buildDir) {
  const sourceDir = resolve(targetBuildDir, 'source')
  return {
    buildDir: targetBuildDir,
    sourceDir,
    assetsDir: resolve(sourceDir, 'assets'),
    pdfPath: resolve(targetBuildDir, 'Folder-Tab-Story-Detail-Card-Pack.pdf'),
    zipPath: resolve(targetBuildDir, 'folder-tab-story-detail-card-pack.zip'),
    htmlPath: resolve(sourceDir, 'folder-tab-story-detail-card-pack.html'),
...
function renderCard(card, index, worlds, imageMap) {
  ...
  if (!world) throw new Error(`Unknown Folder Tab Detail Card world slug: ${card.worldSlug}`)
  const imagePath = imageMap.get(card.worldSlug)
  if (!imagePath) throw new Error(`Missing Folder Tab Detail Card copied image for ${card.worldSlug}`)
  const fields = [
    renderField('Focus detail', card.focusDetailPrompt),
    renderField('Object trait', card.objectTraitPrompt),
    renderField('Place detail', card.placeDetailPrompt),
    renderField('Character move', card.characterActionPrompt),
    renderField('Mood signal', card.moodSignalPrompt),
    renderField('Sentence frame', card.sentenceFramePrompt),
    renderField('Detail question', card.detailQuestionPrompt),
    renderField('Revise detail', card.reviseDetailPrompt),
  ].join('\n')

  return `
    <section class="pack-page story-detail-card-page">
      <div class="page-kicker">Detail Card ${index + 1} | Ages ${escapeHtml(card.ageBand)} | ${escapeHtml(card.detailSkill)}</div>
...
export function renderFolderTabStoryDetailCardPackHtml(source, worlds, imageMap = new Map()) {
  const worldCards = source.worldSlugs
    .map((slug) => {
      const world = worlds.get(slug)
      if (!world) throw new Error(`Unknown Folder Tab Detail Card source world slug: ${slug}`)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .story-detail-card-page { font-size: 7.52px; }
      ...
      .story-detail-card-page figure { float: right; width: 1.34in; margin: 0 0 0.05in 0.09in; }
      .story-detail-card-page figure img { width: 100%; aspect-ratio: 1.28 / 1; object-fit: cover; }
  ...
  <body>
    ...
    <section class="pack-page guide-page">
      ...
      <h2>Detail coaching</h2>
      <div class="guide-grid">
        ${renderGuideCard(source.adultGuide.title, source.adultGuide.bullets)}
      </div>
      <p class="footer-note">Use one folder tab detail card at a time. Keep every choice broad, invented, and screen-free.</p>
    </section>
    <section class="pack-page world-page">
      <p class="page-kicker">World menu</p>
      <h2>Sixteen detail-card worlds</h2>
      <p>Pick a world image, then start one fictional story with a pretend folder tab detail card.</p>
      <div class="world-grid">${worldCards}</div>
    </section>
    <section class="pack-page routine-page">
      <p class="page-kicker">Detail routines</p>
      <h2>Choose the folder tab detail rhythm</h2>
      <div class="routine-grid">${source.detailRoutines.map(renderRoutine).join('\n')}</div>
    </section>
    <section class="pack-page slip-page">
      <p class="page-kicker">Take-home detail slips</p>
      <h2>Start one story detail later</h2>
      <div class="slip-grid">${source.takeHomeDetailSlips.map(renderSlip).join('\n')}</div>
...
export function loadFolderTabStoryDetailCardPackBuildInputs() {
...
function prepareBuildDirectory(paths = folderTabDetailBuildPaths()) {
  rmSync(paths.buildDir, { recursive: true, force: true })
  mkdirSync(paths.assetsDir, { recursive: true })
}

function copyPackAssets(source, paths = folderTabDetailBuildPaths()) {
  const imageMap = new Map()
  for (const slug of new Set(source.worldSlugs)) {
...
function writeReadme(source, paths = folderTabDetailBuildPaths()) {
  const text = [
    source.title,
    '',
    ...
    '',
    'Files:',
    '- Folder-Tab-Story-Detail-Card-Pack.pdf',
    '- source/folder-tab-story-detail-card-pack.html',
    '- source/assets/*.jpg',
    '',
    'Fulfillment note:',
...
function zipEntries(paths = folderTabDetailBuildPaths()) {
  const entries = [
    {
      name: 'Folder-Tab-Story-Detail-Card-Pack.pdf',
    ...
    {
      name: 'source/folder-tab-story-detail-card-pack.html',
...
export async function buildFolderTabStoryDetailCardPack(options = {}) {
  const { source, product, worlds } = loadFolderTabStoryDetailCardPackBuildInputs()
  const paths = folderTabDetailBuildPaths(options.buildDir ?? buildDir)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  buildFolderTabStoryDetailCardPack().catch((error) => {
    console.error(error)
    process.exitCode = 1
  })
}
